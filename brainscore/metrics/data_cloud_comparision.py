...
def get_means_stds(shape: str, assembly: LazyLoad, use_std=True) -> Tuple[np.array, np.array]:
...
'''
def get_avg_slope(outer_i: int, display_sizes:list, cloud_1: np.array, cloud_2: np.array, cloud_3: np.array) -> ndarray:
    slopes = []
...
def generate_human_data(display_sizes: list, shapes_cloud: Dict[int, np.array], cubes_cloud: Dict[int, np.array]) -> list:
