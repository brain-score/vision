...
class BrainModel:
    ...
    def look_at(self, stimuli: Union[StimulusSet, List[str]], number_of_trials=1):
        ...
        :param stimuli: A set of stimuli, passed as either a :class:`~brainio_core.stimuli.StimulusSet`
    ...
    def start_recording(self, recording_target: RecordingTarget, time_bins=List[Tuple[int]]):
        ...
        For all followings call of :meth:`~brainscore.model_interface.BrainModel.look_at`, the model returns the
        corresponding recordings. These recordings are a :class:`~brainio_core.assemblies.NeuroidAssembly` with exactly
        3 dimensions:

        - `presentation`: the presented stimuli (cf. stimuli argument of
          :meth:`~brainscore.model_interface.BrainModel.look_at`). If a :class:`~brainio_core.stimuli.StimulusSet`
          was passed, the recordings should contain all of the :class:`~brainio_core.stimuli.StimulusSet` columns as
