...
def get_firing_rates(model_identifier, model, region, stimulus_identifier, number_of_trials, in_rf):
    ...
    activations = record_from_model(model, stimulus_identifier, number_of_trials).transpose()
...
def map_receptive_field_locations(model_identifier, model: BrainModel, region):
    blank_activations = record_from_model(model, BLANK_STIM_NAME, RF_NUMBER_OF_TRIALS).transpose()
    blank_activations = blank_activations.values
    blank_activations[blank_activations < 0] = 0

    rf_activations = record_from_model(model, RF_STIM_NAME, RF_NUMBER_OF_TRIALS).transpose()
...
def firing_rates_affine(model_identifier, model: BrainModel, region):
    blank_activations = record_from_model(model, BLANK_STIM_NAME, ORIENTATION_NUMBER_OF_TRIALS).transpose()
    orientation_activations = record_from_model(model, ORIENTATION_STIM_NAME, ORIENTATION_NUMBER_OF_TRIALS).transpose()
