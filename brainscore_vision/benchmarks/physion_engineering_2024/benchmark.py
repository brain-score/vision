...
class PhysionGlobalDetectionIntraScenarioAccuracy(BenchmarkBase):
    def __init__(self):
        ...
        self._similarity_metric = load_metric('accuracy')
        ceiling = Score([1, np.nan], coords={'aggregation': ['center', 'error']}, dims=['aggregation'])
        super(PhysionGlobalDetectionIntraScenarioAccuracy, self).__init__(identifier='Physionv1.5-ocd-intra-generalization', version=1,
                                           ceiling_func=lambda: ceiling,
                                           parent='Physion Engineering',
                                           bibtex="""@article{bear2021physion,
                                               title={Physion: Evaluating physical prediction from vision in humans and machines},
...
class PhysionGlobalPredictionIntraScenarioAccuracy(BenchmarkBase):
    def __init__(self):
        ...
        self._similarity_metric = load_metric('accuracy')
        ceiling = Score([1, np.nan], coords={'aggregation': ['center', 'error']}, dims=['aggregation'])
        super(PhysionGlobalPredictionIntraScenarioAccuracy, self).__init__(identifier='Physionv1.5-ocp-intra-generalization', version=1,
                                           ceiling_func=lambda: ceiling,
                                           parent='Physion Engineering',
                                           bibtex="""@article{bear2021physion,
                                               title={Physion: Evaluating physical prediction from vision in humans and machines},
...
class PhysionSnippetDetectionIntraScenarioAccuracy(BenchmarkBase):
    def __init__(self):
        ...
        self._similarity_metric = load_metric('accuracy')
        ceiling = Score([1, np.nan], coords={'aggregation': ['center', 'error']}, dims=['aggregation'])
        super(PhysionSnippetDetectionIntraScenarioAccuracy, self).__init__(identifier='Physionv1.5-snippet-rollout-intra-performance', version=1,
                                           ceiling_func=lambda: ceiling,
                                           parent='PhysionV1.5',
                                           bibtex="""@article{bear2021physion,
                                               title={Physion: Evaluating physical prediction from vision in humans and machines},
...
class PhysionGlobalPredictionPlacementAccuracy(BenchmarkBase):
    def __init__(self):
        self._stimulus_set  = load_stimulus_set("PhysionGlobalPrediction2024")
        self._visual_degrees = 8
        self._similarity_metric = load_metric('accuracy')
        ceiling = Score([1, np.nan], coords={'aggregation': ['center', 'error']}, dims=['aggregation'])
        super(PhysionGlobalPredictionPlacementAccuracy, self).__init__(identifier='Physionv1.5-placement-ocp', version=1,
                                           ceiling_func=lambda: ceiling,
                                           parent='Physion Engineering',
                                           bibtex="""@article{bear2021physion,
                                               title={Physion: Evaluating physical prediction from vision in humans and machines},
    ...
    def __call__(self, candidate):
        # prepare fitting stimuli
        fitting_stimuli = self._stimulus_set[self._stimulus_set['contacts'] != -1]
        fitting_stimuli = fitting_stimuli[fitting_stimuli['train'] == 1]
        fitting_stimuli = place_on_screen(fitting_stimuli, target_visual_degrees=candidate.visual_degrees(),
                                          source_visual_degrees=self._visual_degrees)
        # prepare test stimuli
        test_stimuli = self._stimulus_set[self._stimulus_set['contacts'] != -1]
        test_stimuli = test_stimuli[test_stimuli['train'] == 0]
        test_stimuli = place_on_screen(test_stimuli, target_visual_degrees=candidate.visual_degrees(),
                                          source_visual_degrees=self._visual_degrees)
        ...
        score = self._similarity_metric(
            predictions['choice'],
            test_stimuli['contacts']
        )
        return score


class PhysionGlobalDetectionPlacementAccuracy(BenchmarkBase):
    def __init__(self):
        self._stimulus_set  = load_stimulus_set("PhysionGlobalDetection2024")
        self._visual_degrees = 8
        self._similarity_metric = load_metric('accuracy')
        ceiling = Score([1, np.nan], coords={'aggregation': ['center', 'error']}, dims=['aggregation'])
        super(PhysionGlobalDetectionPlacementAccuracy, self).__init__(identifier='Physionv1.5-placement-ocd', version=1,
                                           ceiling_func=lambda: ceiling,
                                           parent='Physion Engineering',
                                           bibtex="""@article{bear2021physion,
                                               title={Physion: Evaluating physical prediction from vision in humans and machines},
...
class PhysionGlobalDetectionIntraScenarioPlacementAccuracy(BenchmarkBase):
    def __init__(self):
        self._stimulus_set  = load_stimulus_set("PhysionGlobalDetection2024")
        self._visual_degrees = 8
        self._similarity_metric = load_metric('accuracy')
        ceiling = Score([1, np.nan], coords={'aggregation': ['center', 'error']}, dims=['aggregation'])
        super(PhysionGlobalDetectionIntraScenarioPlacementAccuracy, self).__init__(identifier='Physionv1.5-ocd-placement-intra-generalization', version=1,
                                           ceiling_func=lambda: ceiling,
                                           parent='Physion Engineering',
                                           bibtex="""@article{bear2021physion,
                                               title={Physion: Evaluating physical prediction from vision in humans and machines},
...
class PhysionGlobalPredictionIntraScenarioPlacementAccuracy(BenchmarkBase):
    def __init__(self):
        self._stimulus_set  = load_stimulus_set("PhysionGlobalPrediction2024")
        self._visual_degrees = 8
        self._similarity_metric = load_metric('accuracy')
        ceiling = Score([1, np.nan], coords={'aggregation': ['center', 'error']}, dims=['aggregation'])
        super(PhysionGlobalPredictionIntraScenarioPlacementAccuracy, self).__init__(identifier='Physionv1.5-ocp-placement-intra-generalization', version=1,
                                           ceiling_func=lambda: ceiling,
                                           parent='Physion Engineering',
                                           bibtex="""@article{bear2021physion,
                                               title={Physion: Evaluating physical prediction from vision in humans and machines},
...
class PhysionSnippetDetectionPlacementAccuracy(BenchmarkBase):
    def __init__(self):
        self._stimulus_set  = load_stimulus_set("PhysionSnippetDetection2024")
        self._visual_degrees = 8
        self._similarity_metric = load_metric('accuracy')
        ceiling = Score([1, np.nan], coords={'aggregation': ['center', 'error']}, dims=['aggregation'])
        super(PhysionSnippetDetectionPlacementAccuracy, self).__init__(identifier='Physionv1.5-snippet-placement-performance', version=1,
                                           ceiling_func=lambda: ceiling,
                                           parent='PhysionV1.5',
                                           bibtex="""@article{bear2021physion,
                                               title={Physion: Evaluating physical prediction from vision in humans and machines},
    ...
    def __call__(self, candidate):
        # prepare fitting stimuli
        fitting_stimuli = self._stimulus_set[self._stimulus_set['contacts'] != -1]
        fitting_stimuli = self._stimulus_set[fitting_stimuli['train'] == 1]
        fitting_stimuli = place_on_screen(fitting_stimuli, target_visual_degrees=candidate.visual_degrees(),
                                          source_visual_degrees=self._visual_degrees)
        # prepare test stimuli
        test_stimuli = self._stimulus_set[self._stimulus_set['contacts'] != -1]
...
class PhysionSnippetDetectionPlacementIntraScenarioAccuracy(BenchmarkBase):
    def __init__(self):
        self._stimulus_set  = load_stimulus_set("PhysionSnippetDetection2024")
        self._visual_degrees = 8
        self._similarity_metric = load_metric('accuracy')
        ceiling = Score([1, np.nan], coords={'aggregation': ['center', 'error']}, dims=['aggregation'])
        super(PhysionSnippetDetectionPlacementIntraScenarioAccuracy, self).__init__(identifier='Physionv1.5-snippet-placement-intra-performance', version=1,
                                           ceiling_func=lambda: ceiling,
                                           parent='PhysionV1.5',
                                           bibtex="""@article{bear2021physion,
                                               title={Physion: Evaluating physical prediction from vision in humans and machines},
