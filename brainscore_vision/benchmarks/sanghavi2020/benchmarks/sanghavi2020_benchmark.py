...
from brainscore_vision.data.sanghavi2020 import BIBTEX_SANGHAVI
...
def _DicarloSanghavi2020Region(region):
    ...
    return NeuralBenchmark(identifier=f'dicarlo.Sanghavi2020.{region}-pls', version=1,
                           assembly=assembly, similarity_metric=metric,
                           visual_degrees=VISUAL_DEGREES, number_of_trials=NUMBER_OF_TRIALS,
                           ceiling_func=lambda: ceiler(assembly_repetition),
                           parent=region,
                           bibtex=BIBTEX_SANGHAVI)
...
def load_assembly(average_repetitions, region):
    assembly = load_dataset(f'dicarlo.Sanghavi2020')
