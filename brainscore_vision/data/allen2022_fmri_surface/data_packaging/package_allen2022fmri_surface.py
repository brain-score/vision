...
DEFAULT_BUCKET_NAME = "brainscore-storage/brainscore-vision/benchmarks/Allen2022_fmri_surface"
...
def load_assembly(brainscore_dir: Path, split: str) -> NeuroidAssembly:
    ...
    path = brainscore_dir / f"Allen2022_fmri_surface_{split}.nc"
    da = xr.open_dataarray(str(path))
    da.load()

    assembly = NeuroidAssembly(da)
    assembly.name = f"Allen2022_fMRI_surface_{split}_Assembly"
    return assembly
...
def build_stimulus_set(brainscore_dir: Path, split: str) -> StimulusSet:
    ...
    stimulus_set.name = f"Allen2022_fMRI_surface_{split}_Stimuli"
    stimulus_set.identifier = f"Allen2022_fMRI_surface_{split}_Stimuli"
    return stimulus_set
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Package Allen2022 NSD surface fMRI data for Brain-Score")
    parser.add_argument(
        "--brainscore-dir",
        type=str,
