...
class KatoErrorConsistency(Metric):
    """
    ...
    def compare_single_subject(self, source, target):
        ...
        assert all(source['stimulus_id'].values == target['stimulus_id'].values) # start from "acorn" -> "airplane"...
        ...
        observed_consistency = (correct_source == correct_target).sum() / len(correct_source)
