...
from brainscore_vision.metric_helpers.xarray_utils import XarrayRegression
        ...
        with self._graph.as_default():
            with tf.compat.v1.variable_scope('loss'):
                ...
                laplace_loss = tf.reduce_sum(
                    input_tensor=[tf.norm(
                        tensor=tf.nn.conv2d(input=t, filters=laplace_filter, strides=[1, 1, 1, 1], padding='SAME')) for
                                  t in self._s_vars])
                ...
                self.reg_loss = self._ls * (l2_loss + laplace_loss) + \
                                self._ld * tf.reduce_sum(
                    input_tensor=[tf.reduce_sum(input_tensor=tf.pow(t, 2)) for t in self._d_vars])
        ...
        with self._graph.as_default():
            ...
            self._sess = tf.compat.v1.Session(
                config=tf.compat.v1.ConfigProto(
                    gpu_options=self._gpu_options) if self._gpu_options is not None else None)
