...
from brainscore_vision.model_helpers.activations.temporal.core.inferencer.video import OnlineTemporalInferencer
...
class VideoReadoutMapping(BrainModel):
    ...
    def start_task(self, task: BrainModel.Task, fitting_stimuli, simulation=False):
        ...
        self.current_task = task
        fitting_stimuli['train_flag'] = [True]*len(fitting_stimuli['stimulus_id'])
        fitting_features = self.activations_model(fitting_stimuli, layers=self.readout)
        assert all(fitting_features['stimulus_id'].values == fitting_stimuli['stimulus_id']), \
            "stimulus_id ordering is incorrect"
        fitting_features = fitting_features.transpose('presentation', 'time_bin', 'neuroid')
        self.classifier = VideoReadoutMapping.TransformerReadout(np.prod(fitting_features.shape[2:]),
                                                                 self.num_classes)
        
        if not isinstance(self.activations_model._extractor.inferencer, OnlineTemporalInferencer):
            self.n_time_bins = fitting_features.shape[1]
            if self.num_classes == 1:
                self.classifier.fit(fitting_features, 
                                fitting_features['label'].values)
            else:
                self.classifier.fit(fitting_features, 
                                fitting_features['contacts'].values)

    def look_at(self, stimuli, number_of_trials=1, require_variance=False, simulation=False):
        stimuli['train_flag'] = [False]*len(stimuli['stimulus_id'])
        if not simulation:    
    ...
    class TransformerReadout:
        def __init__(self, model_dim, num_classes=1):
            ...
            self.model = VideoReadoutMapping.ReadoutModel(model_dim, num_classes=num_classes)
            self.num_classes = num_classes
        ...
        def train(self, data_loader, optimizer, criterion, 
            ...
            for batch_idx, data in enumerate(data_loader):
                ...
                optimizer.zero_grad()
                outputs = self.model(inputs)  # Ensure the output is of correct shape
        ...
        def predict(self, features):
            features = features.transpose('presentation', 'time_bin', 'neuroid')
            test_loader, _ = self.build_loader(features, None, mode='test')
            ...
            proba = BehavioralAssembly(proba,
                                       ...
                                       {'stimulus_id': ('presentation', features['stimulus_id'].data),
                                        'label': ('presentation', features['label'].data),
    ...
    class ReadoutModel(nn.Module):
        def __init__(self, input_dim, embed_dim=252, num_classes=1):
