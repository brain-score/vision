...
class RecurrentCNN(keras.Model):
    def __init__(self, fb_loops=0):
        ...
        self.V2 = RecurrentCNNBlock(64, times=1, use_fb=True, us_factor=2)
        self.mp1 = layers.MaxPooling2D(pool_size=(2,2), strides=(2,2), padding="same")
        self.V4 = RecurrentCNNBlock(128, times=1, use_fb=True, us_factor=2, pool_factor=8)
        self.mp2 = layers.MaxPooling2D(pool_size=(2,2), strides=(2,2), padding="same")
        self.IT = RecurrentCNNBlock(256, times=1, use_fb=False)
...
def get_model_list():
    return ['sketch_model_10f-ep23']


def get_model(name):
    assert name == 'sketch_model_10f-ep23'

    model = RecurrentCNN(fb_loops=8)
    model.load_weights(os.path.join(os.path.dirname(__file__), 'rcnn_cand_10f_weights_23ep/'))
