...
class hGRU_V1(layers.Layer):
    def __init__(self, out_channels):
        ...
        self.conv_fb2 = layers.Conv2DTranspose(out_channels, kernel_size=7, strides=(4,4), padding="same", use_bias=False)
...
def get_model_list():
    return ['sketch_model_11-ep20']


def get_model(name):
    assert name == 'sketch_model_11-ep20'

    model = RecurrentCNN(fb_loops=4)
    model.load_weights(os.path.join(os.path.dirname(__file__), 'rcnn_cand_11_weights_20ep/'))
