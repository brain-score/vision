...
class RecurrentCNN(keras.Model):
    def __init__(self, fb_loops=0):
        ...
        self.V2 = RecurrentCNNBlock(128, times=1)
        self.mp1 = layers.MaxPooling2D(pool_size=(2,2), strides=(2,2), padding="same")
        self.V4 = RecurrentCNNBlock(256, times=1, pool_factor=8)
...
def get_model_list():
    return ['sketch_model_12-ep25']


def get_model(name):
    assert name == 'sketch_model_12-ep25'

    model = RecurrentCNN(fb_loops=4)
    model.load_weights(os.path.join(os.path.dirname(__file__), 'rcnn_cand_12_weights_25ep/'))
