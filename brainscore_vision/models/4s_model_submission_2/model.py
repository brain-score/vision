...
def get_model_list():
    return ['sketch_model-4s-ep1-weights']


def get_model(name):
    assert name == 'sketch_model-4s-ep1-weights'

    model = RecurrentCNN(fb_loops=4)
    model.load_weights(os.path.join(os.path.dirname(__file__), 'rcnn_cand_4s_weights_1ep/'))
...
def get_layers(name):
    ...
    return [
        ...
        "conv2d_1", "batch_normalization_1", "activation_1", "activation_2", "conv2d_transpose", "conv2d_transpose_1",
        "batch_normalization_2", "conv2d_2", "recurrent_cnn_block", "max_pooling2d_1", "recurrent_cnn_block_1",
        "max_pooling2d_2", "recurrent_cnn_block_2", "global_average_pooling2d"
        ]
