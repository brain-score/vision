...
class RecurrentCNNBlock(layers.Layer):
    ...
    def __init__(self, out_channels, times=1, use_fb=False, use_fb2=False, fb_channels=None, us_factor=None, fb2_channels=None, us2_factor=None):
        ...
        self.norm_skip = layers.Lambda(lambda x: tf.nn.local_response_normalization(x, alpha=.0001))
        ...
        self.norm_fb = layers.Lambda(lambda x: tf.nn.local_response_normalization(x, alpha=.0001))
...
class RecurrentCNN(keras.Model):
    def __init__(self, fb_loops=0):
        ...
        self.V1 = [
            ...
            layers.Lambda(lambda x: tf.nn.local_response_normalization(x, alpha=.0001)),
        ...
        self.V1_norm_fb = layers.Lambda(lambda x: tf.nn.local_response_normalization(x, alpha=.0001)),
...
def get_model_list():
    return ['sketch_model_4w-ep20']


def get_model(name):
    assert name == 'sketch_model_4w-ep20'
