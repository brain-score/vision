...
class RecurrentCNN(keras.Model):
    ...
    def call(self, x, training=None, mask=None):
        ...
        for t in range(self.fb_loops-1):
            x = v1_response + self.V1_conv_lat(v1_state) + self.V1_conv_fb2(self.V4.hidden_state)# + self.V1_conv_fb2(self.V4.hidden_state)
            x = self.V1_norm_fb(x)
            x = self.nonlin(x)
            v1_state = x
            # x = model.V1_Lam(x)
...
def get_model_list():
    return ['sketch_model_5l-ep19']


def get_model(name):
    assert name == 'sketch_model_5l-ep19'

    model = RecurrentCNN(fb_loops=8)
    model.load_weights(os.path.join(os.path.dirname(__file__), 'rcnn_cand_5l_weights_19ep/'))
    ...
    for t in range(model.fb_loops-1):
        x = v1_response + model.V1_conv_lat(v1_state) + model.V1_conv_fb2(model.V4.hidden_state)# + model.V1_conv_fb2(model.V4.hidden_state)
    ...
    #   For final pass, include lambda identity layers in hopes that these final values are used rather than ones earlier.
    x = v1_response + model.V1_conv_lat(v1_state) + model.V1_conv_fb2(model.V4.hidden_state)# + self.V1_conv_fb2(self.V4.hidden_state)
    x = model.V1_norm_fb(x)
    x = model.nonlin(x)
    # x = self.V1_norm_fb(x)
    x = model.V1_Lam(x)

    x = model.V2(x)#, td_inp=self.V4.hidden_state)
    x = model.mp1(x)
    x = model.V2_Lam(x)
    # x = v4_response + self.V4.conv_lat(v4_state) + self.V4.conv_fb(self.IT.hidden_state)
    # x = self.V4.norm_fb(x)
    # x = self.nonlin(x)
    # v4_state = x
    x = model.V4(x)#, td_inp=self.IT.hidden_state)
    x = model.mp2(x)
    x = model.V4_Lam(x)
    x = model.IT(x)
    x = model.IT_Lam(x)
