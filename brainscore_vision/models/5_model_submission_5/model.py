...
class RecurrentCNN(keras.Model):
    ...
    def call(self, x, training=None, mask=None):
        ...
        for t in range(self.fb_loops):
            x = v1_response + self.V1_conv_lat(v1_state)# + self.V1_conv_fb1(self.V2.hidden_state) + self.V1_conv_fb2(self.V4.hidden_state)
            x = self.V1_norm_fb(x)
            x = self.nonlin(x)
            v1_state = x
            x = model.V1_Lam(x)
...
def get_model_list():
    return ['sketch_model_5k-ep8']


def get_model(name):
    assert name == 'sketch_model_5k-ep8'

    model = RecurrentCNN(fb_loops=8)
    model.load_weights(os.path.join(os.path.dirname(__file__), 'rcnn_cand_5k_weights_8ep/'))
    ...
    for t in range(model.fb_loops):
        x = v1_response + model.V1_conv_lat(v1_state)# + model.V1_conv_fb1(model.V2.hidden_state) + model.V1_conv_fb2(model.V4.hidden_state)
