...
class RecurrentCNN(keras.Model):
    def __init__(self, fb_loops=0):
        ...
        self.V1_conv_fb2 = layers.Conv2DTranspose(64, kernel_size=3, strides=(14,14), padding="same", use_bias=False)
        ...
        self.V4 = RecurrentCNNBlock(256, times=1, use_fb=True, fb_channels=256, us_factor=7)
        self.mp2 = layers.MaxPooling2D(pool_size=(7,7), padding="same", name="V4")
...
def get_model_list():
    return ['sketch_model_6a-ep12']


def get_model(name):
    assert name == 'sketch_model_6a-ep12'

    model = RecurrentCNN(fb_loops=4)
    model.load_weights(os.path.join(os.path.dirname(__file__), 'rcnn_cand_6a_weights_12ep/'))
