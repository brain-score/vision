...
class RecurrentCNN(keras.Model):
    def __init__(self, fb_loops=4):
        ...
        self.V4 = RecurrentCNNBlock(256, times=4, use_fb=True, fb_channels=256, us_factor=2)
    ...
    def call(self, x, training=None, mask=None):
        ...
        for t in range(self.fb_loops-1):
            ...
            #    self.V1_conv_fb2(self.V4.hidden_state)])#, self.V1_conv_fb3(self.IT.hidden_state)
            #])
            x = self.V1_norm_fb(x)
            x = self.nonlin(x)
            v1_state = x
...
def get_model_list():
    return ['sketch_model_7-ep17']


def get_model(name):
    assert name == 'sketch_model_7-ep17'

    model = RecurrentCNN(fb_loops=4)
    model.load_weights(os.path.join(os.path.dirname(__file__), 'rcnn_cand_7_weights_17ep/'))
