...
class RecurrentCNN(keras.Model):
    def __init__(self, fb_loops=0):
        ...
        self.V1_conv_fb1 = layers.Conv2DTranspose(64, kernel_size=7, strides=(2,2), padding="same", use_bias=False)
        self.V1_conv_fb2 = layers.Conv2DTranspose(64, kernel_size=7, strides=(4,4), padding="same", use_bias=False)
        self.V1_conv_lat = layers.Conv2D(64, kernel_size=7, padding="same", use_bias=False)
        self.V1_mp = layers.MaxPooling2D(pool_size=(2,2), strides=(2,2), padding="same")
        ...
        self.V4 = RecurrentCNNBlock(256, times=4, use_fb=True, us_factor=2, pool_factor=4)
...
def get_model_list():
    return ['sketch_model_7c-ep18']


def get_model(name):
    assert name == 'sketch_model_7c-ep18'

    model = RecurrentCNN(fb_loops=4)
    model.load_weights(os.path.join(os.path.dirname(__file__), 'rcnn_cand_7c_weights_18ep/'))

    inputs = keras.Input(shape=(224, 224, 3))
    x = inputs

    x = model.V1_conv_inp(x)
    ...
    x = model.V2(x)
    x = model.mp1(x)
    x = model.V4(x, skip_inp=v1_state)
    x = model.mp2(x)
    model.IT(x)
    ...
    for t in range(model.fb_loops-1):
        x = model.V1_Add([
            v1_init_response,
            model.V1_conv_lat(v1_state),
            model.V1_conv_fb1(model.V2.hidden_state),
