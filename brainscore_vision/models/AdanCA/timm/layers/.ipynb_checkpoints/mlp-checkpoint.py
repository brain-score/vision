...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        if(energy_minimization):
            self.energy_coeff = nn.Parameter(torch.zeros(1) + 0.001)
            self.perception = nn.Conv2d(in_features, in_features, 3, padding=1, groups=in_features//n_groups)
    ...
    def forward(self, x):
        if(self.energy_minimization):
            with torch.enable_grad():
                ...
                B,N,C = x.shape
                x_percept = x.reshape(B, int(N**0.5), int(N**0.5), C).permute(0,3,1,2)
                y_percept = self.perception(x_percept)
                y_percept = y_percept.permute(0,2,3,1).reshape(B, -1, C)
#                 x = self.fc1(x)
#                 x = self.act(x)
#                 x = self.drop1(x)
#                 energy = self.norm(x)
                with torch.cuda.amp.autocast(enabled = False):
                    ...
                    energy = self.norm(self.drop1(self.act(self.fc1(y_percept))))
                    ...
                    y1 = -grad_energy[0]
                y2 = self.drop2(self.fc2(energy))
                x = self.energy_coeff * y1 + y2
        else:
            x = self.fc1(x)
            x = self.act(x)
            x = self.drop1(x)
