...
class PatchEmbedOverLap(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Sequential(
            nn.Conv2d(in_chans, 32, kernel_size=(7, 7), stride=(2, 2), padding=(2, 2)), # 111
            nn.BatchNorm2d(32),
            nn.MaxPool2d(3, stride=2, padding=1),
            nn.Conv2d(32, embed_dim, kernel_size=(1, 1), stride=(1, 1))
        )
        # nn.Conv2d(in_chans, embed_dim, kernel_size=(2*patch_size-1), stride=patch_size, padding = patch_size - 1, bias=bias)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()
