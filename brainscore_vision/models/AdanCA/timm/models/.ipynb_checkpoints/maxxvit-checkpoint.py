...
class MaxxVitBlock(nn.Module):
    ...
    def __init__(
        ...
        self.nca_norm = nca_norm
        self.nca_drop_path = nca_drop_path
...
#         if(not self.test_ori):
#             x = self.conv(x)
        
        if(self.before_conv):
            x = self.conv(x)
    
        if(self.nca_model is not None):
            B,C,H,W = x.shape
            x = x.permute(0,2,3,1).reshape(B,H*W,C)
            nca_output, _ = self.nca_model(self.nca_norm(x.contiguous()).contiguous())
            nca_output = self.nca_drop_path(nca_output)
            x = x + nca_output
            x = x.reshape(B,H,W,C).permute(0,3,1,2)
        
        #if(self.test_ori):
        if(not self.before_conv):
            x = self.conv(x)
...
class MaxxVitStage(nn.Module):
    def __init__(
        ...
        if(self.nca_model):
            ...
            if(test_ori):
                nca_in_chs = 1
            self.nca_delay = delay
            self.nca_drop_path = DropPath(drop_path[delay]) if drop_path[delay] > 0. else nn.Identity()
            nca_dim = in_chs * nca_in_chs if in_chs != 64 else 96
            self.nca_norm = nn.LayerNorm(nca_dim) if before_nca_norm else nn.Identity()
            self.nca = NCA(
                dim = nca_dim,
                num_heads = nca_dim // transformer_cfg.dim_head,
                act_layer=nn.GELU,
                norm_layer=None,
                separate_norm = False,
                ...
                perception_scales = [0,1] if msp else [0],
                pos_emb = None,
            ...
            print("additional FLOPs:", self.nca.flops(verbose = True) / 1e9)
        ...
        for i, t in enumerate(block_types):
            ...
            if t == 'C':
                ...
                blocks += [MaxxVitBlock(
                    ...
                    drop_path=drop_path[i],
                    nca_model = self.nca if i == self.nca_delay else None,
                    nca_norm = self.nca_norm if i == self.nca_delay else None,
                    nca_drop_path = self.nca_drop_path if i == self.nca_delay else None,
                    test_ori = test_ori,
...
class MaxxVit(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_stages):
            ...
            if(nca_model == 1):
                test_ori = 1
                nca_on = 1 if i > 0 else 0
                nca_delay = 0
            elif(nca_model == 2):
                if(i > 1):
                    nca_on = 1
                    nca_delay = 1 if i == 2 else 0
                else:
                    nca_on = 0
                    nca_delay = 0
            elif(nca_model == 3):
                ...
                nca_on = 1 if i != 1 else 0
                nca_delay = 1 if i == 0 or i == 2 else 0
            elif(nca_model == 20):
                nca_on = 0
                nca_delay = 0
                if(i == 2):
