...
class SwinTransformerV2Block(nn.Module):
    ...
    def forward(self, x):
        ...
        if(self.nca_local_perception_model is not None):
            ...
            x = x + self.drop_path1(self.norm1(perception_local_global))
...
class SwinTransformerV2Stage(nn.Module):
    ...
    def __init__(
        ...
        if(self.nca_model):
            ...
            self.nca_drop_path = DropPath(drop_path[0]) if drop_path[0] > 0. else nn.Identity() # drop_path now is a list for all Swin Blocks. So for preprocessing, just use the first drop path
            self.nca_norm1 = norm_layer(out_dim)# if before_nca_norm else nn.Identity()
            self.nca_norm2 = norm_layer(out_dim)
            self.nca = NCA(
                dim = out_dim,
                num_heads = num_heads,
                act_layer=nn.GELU,
                ...
                perception_scales = [0, 1] if nca_model == 1 else [0],
                pos_emb = None,
            ...
            print("additional FLOPs:", self.nca.flops(verbose = True) / 1e9, self.nca_drop_path)
    ...
    def forward(self, x):
        ...
        if(self.nca_model):
            B,H,W,C = x.shape
            x = x.reshape(B,H*W,C)
            nca_output, _ = self.nca(self.nca_norm1(x).contiguous())
            nca_output = self.nca_drop_path(self.nca_norm2(nca_output))
            x = x + nca_output
            x = x.reshape(B,H,W,C)
        ...
        for i in range(self.num_layers):
            ...
            if(nca_model):
                layers += [SwinTransformerV2Stage(
                    ...
                    nca_model = nca_model if i > 0 else 0,
