...
#             self.nca_preprocess_apply_layer = [depth // nca_model * x for x in range(1, nca_model)]
    ...
    def forward_features(self, x, return_middle = False):
        B = x.shape[0]
        if(return_middle):
            middle_dict = {}
        ...
        if(self.nca_model and self.nca_model < 20 and self.nca_model != 18):
            if(self.before_nca_norm):
                nca_output, _ = self.nca_preprocess[0](self.nca_norm_preprocess[0](x.contiguous()).contiguous())
            else:
                nca_output, _ = self.nca_preprocess[0](x.contiguous())
...
#                 nca_output = self.nca_drop_path(nca_output)
                ...
                if(self.nca_model):
                    if(blk_idx in self.nca_preprocess_apply_layer):
                        if(self.before_nca_norm):
                            ...
                            nca_output, _ = self.nca_preprocess[nca_idx](x.contiguous())
    ...
    def forward(self, x, return_middle = False):
        x = self.forward_features(x, return_middle)
        if(self.adl_loss):
            x, attn_list = x
        if(return_middle):
            x, middle_dict = x
