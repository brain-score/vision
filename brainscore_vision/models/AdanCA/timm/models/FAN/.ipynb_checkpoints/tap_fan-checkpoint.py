...
class ChannelProcessing(nn.Module):
    ...
    def forward(self, x, H, W, atten=None):
        ...
        Bv, Hd, Nv, Cv = v.shape
        v = self.norm_v(self.mlp_v(v.transpose(1, 2).contiguous().reshape(Bv, Nv, Hd*Cv), H, W).contiguous()).reshape(Bv, Nv, Hd, Cv).transpose(1, 2)
...
class FAN(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12, sharpen_attn=False, channel_dims=None,
                 ...
                 act_layer=None, norm_layer=None,se_mlp=False, cls_attn_layers=2, use_pos_embed=True, eta=1., tokens_norm=False, c_head_num=None, hybrid_patch_size=2, head_init_scale=1.0, num_scales=1, return_attn = 0,):
...
def tap_fan_base_16_p4_hybrid(pretrained=False, **kwargs):
    ...
    backbone = _create_hybrid_backbone(pretrained=False, pretrained_strict=False, **model_args)
    model_kwargs = dict(
        patch_size=16, embed_dim=448, depth=depth, num_heads=8, eta=1.0, tokens_norm=True, sharpen_attn=False, num_scales=4, **kwargs)
