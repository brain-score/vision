...
from timm.models.helpers import named_apply, build_model_with_cfg_old
...
class ConvNeXtStage(nn.Module):
    ...
    def __init__(
        ...
        if(self.nca_model):
            self.nca_drop_path = DropPath(dp_rates[0]) if dp_rates[0] > 0. else nn.Identity()
            self.nca_norm = norm_layer(out_chs) if before_nca_norm else nn.Identity()
            self.nca = NCA(
                dim = out_chs,
                num_heads = 3,
                act_layer=nn.GELU,
    ...
    def forward(self, x):
        ...
        if(self.nca_model):
            B,C,H,W = x.shape
            nca_norm_output = self.nca_norm(x.contiguous())
            
            nca_norm_output = nca_norm_output.permute(0,2,3,1).reshape(B,H*W,C)
            
            nca_output, _ = self.nca(nca_norm_output.contiguous())
            
            nca_output = self.nca_drop_path(nca_output)
            
            nca_output = nca_output.reshape(B,H,W,C).permute(0,3,1,2)
            x = x + nca_output
