...
from timm.models.helpers import build_model_with_cfg_old
...
class PositionalEncodingFourier(nn.Module):
    ...
    def forward(self, B: int, H: int, W: int):
        ...
        pos = self.token_projection(pos.contiguous())
...
class Mlp(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.linear:
            x = self.relu(x)
        x = self.drop(self.gamma * self.dwconv(x.contiguous(), H, W)) + x
        x = self.fc2(x)
        x = self.drop(x)
        return x
class PerceptionAggr(nn.Module):
    def __init__(self, dim, num_scales, multi_head_combine = False, head_num = 1, scale_weight = False):
        ...
        self.proj = nn.Sequential(
            nn.Conv2d(dim, dim//2, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups = head_num, bias=False),
            nn.BatchNorm2d(dim//2),
            
        )
        """1 kernel will cause the bucket grad error!!!"""
        self.aggr = nn.Conv2d(dim//2, num_scales*head_num, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups = head_num, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.aggr(x.contiguous())
        x = x.permute(0, 2, 3, 1).contiguous() # B,H,W,K
        x = x.unsqueeze(1).contiguous()
        x = x.softmax(dim=-1).contiguous()
...
class FANBlock(nn.Module):
    ...
    def forward(self, x, attn=None, return_attention=False):
        ...
        if(self.nca_local_perception_model is not None):
            ...
            global_perception, attn_s = self.attn(x_norm, H, W)
            ...
            if(mes_time):
                ...
                start = time.time()
            x = x + self.drop_path(self.gamma1 * perception_local_global)
        else:
...
class FAN(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12, sharpen_attn=False, channel_dims=None,
        ...
        if(self.nca_model):
            ...
            #self.nca_drop_path2 = DropPath(drop_path_rate)
            nca_norm_preprocess = []
            nca_preprocess = []
            ...
            msp = [0,1]
            if(ablation_msp):
                print("MSP Ablation",ablation_msp)
                if(ablation_msp == 1):
                    msp = [0]
            
            if(nca_model == 1):
                nca_model_num = 3
                self.nca_preprocess_apply_layer = [5, 10]
            if(nca_model == 3 or nca_model == 5):
                nca_model_num = 2 if nca_model == 3 else 1
                self.nca_preprocess_apply_layer = [5]
            elif(nca_model == 4 or nca_model == 7 or nca_model == 8 or nca_model == 10):
                nca_model_num = 3
                self.nca_preprocess_apply_layer = [5, 10]
            elif(nca_model == 9):
                nca_model_num = 4
                ...
                nca_model_num = 3
                self.nca_preprocess_apply_layer = [5, 9]
#             self.nca_preprocess_apply_layer = [depth // nca_model * x for x in range(1, nca_model)]
            elif(nca_model == 11):
                nca_model_num = 3
                self.nca_preprocess_apply_layer = [5, 11]
            elif(nca_model == 12):
                nca_model_num = 2
                self.nca_preprocess_apply_layer = [6]
            elif(nca_model == 16):
                nca_model_num = 3
                self.nca_preprocess_apply_layer = [6, 9]
            elif(nca_model == 17):
                nca_model_num = 3
                self.nca_preprocess_apply_layer = [6, 9]
                ...
                nca_model_num = 3
                self.nca_preprocess_apply_layer = [1, 6, 9]
                self.nca_drop_path = nn.Identity()
            elif(nca_model >= 20):
                nca_model_num = 1
                self.nca_preprocess_apply_layer = [nca_model - 20]
            print(self.nca_preprocess_apply_layer, self.nca_drop_path)
            self.nca_model = nca_model
            self.before_nca_norm = before_nca_norm
            for i in range(nca_model_num):
                nca_norm_preprocess += [norm_layer(embed_dim)]
                cur_stochastic_update = stochastic_update
                if(nca_model == 12):
                    cur_stochastic_update = stochastic_update * i
                if(nca_model == 16):
                    cur_stochastic_update = stochastic_update
                if(nca_model == 17):
                    cur_stochastic_update = stochastic_update #/ (nca_model_num) * (i+1)
                nca_preprocess += [
                    NCA(
                        dim = embed_dim,
                ...
                print("additional FLOPs:", nca_preprocess[-1].flops(verbose = True) / 1e9, cur_stochastic_update)
        ...
        if(avg_pool):
            ...
            self.avg_pool_weight_layer = PerceptionAggr(self.embed_dim, num_scales=1, multi_head_combine=False, head_num=1, scale_weight = False)
        else:
    ...
    def forward_features(self, x, return_middle = False, return_extra = False):
        B = x.shape[0]
        if(return_middle):
            middle_dict = {}
        x, (Hp, Wp) = self.patch_embed(x.contiguous())

        if self.use_pos_embed:
            pos_encoding = self.pos_embed(B, Hp, Wp).reshape(B, -1, x.shape[1]).permute(0, 2, 1)
            x = x + pos_encoding.contiguous()
        ...
        if(return_extra):
            nca_middle_dict = {}
        if(self.nca_model and self.nca_model < 20 and self.nca_model != 18):
            if(self.before_nca_norm):
                nca_output, nca_middle_output = self.nca_preprocess[0](self.nca_norm_preprocess[0](x.contiguous()).contiguous())
            else:
                nca_output, nca_middle_output = self.nca_preprocess[0](x.contiguous())
                nca_output = self.nca_norm_preprocess[0](nca_output.contiguous())
#             if(self.nca_model != 7 and self.nca_model != 8 and self.nca_model != 9):
#                 nca_output = self.nca_drop_path(nca_output)
        ...
        for blk_idx, blk in enumerate(self.blocks):
            if(return_middle):
                middle_dict[f"block.{blk_idx}"] = x
            blk.H, blk.W = H, W
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x)
            else:
                if(self.nca_model):
                    if(blk_idx in self.nca_preprocess_apply_layer):
                        if(self.before_nca_norm):
                            nca_output, nca_middle_output = self.nca_preprocess[nca_idx](self.nca_norm_preprocess[nca_idx](x.contiguous()).contiguous())
                        else:
                            nca_output, nca_middle_output = self.nca_preprocess[nca_idx](x.contiguous())
                            nca_output = self.nca_norm_preprocess[nca_idx](nca_output.contiguous())
                            
                        # nca_output, _ = self.nca_preprocess[nca_idx](self.nca_norm_preprocess[nca_idx](x.contiguous()).contiguous())
                        if(self.nca_model == 17):
                            nca_output = self.nca_drop_path(nca_output)
                        x = x + nca_output
                        if(return_extra):
                            nca_middle_state = nca_middle_output["middle_state"]
                ...
                if(self.adl_loss):
                    x, attn = blk(x, return_attention = self.adl_loss)
                    attn_list.append(attn)
                elif(not self.adl_loss):
                    x = blk(x, return_attention = self.adl_loss)
            H, W = blk.H, blk.W

        if(self.nca_model == 9):
            if(self.before_nca_norm):
                nca_output, _ = self.nca_preprocess[-1](self.nca_norm_preprocess[-1](x.contiguous()).contiguous())
            else:
                nca_output, _ = self.nca_preprocess[-1](x.contiguous())
                nca_output = self.nca_norm_preprocess[-1](nca_output.contiguous())
            # nca_output, _ = self.nca_preprocess[-1](self.nca_norm_preprocess[-1](x.contiguous()).contiguous())
            # nca_output = self.nca_drop_path(nca_output)
            x = x + nca_output
        ...
        if(self.avg_pool):
            ...
            if(H * W != n_tokens):
                ...
                exit()
            x = x.transpose(1, 2).contiguous().reshape(b_x, c_x, H, W).contiguous()
            avg_weight = self.avg_pool_weight_layer(x.contiguous())[..., 0] # B,1,H,W,1
            x = avg_weight * x.contiguous()
            if(return_middle):
                middle_dict[f"block_output"] = x
            x = self.norm(self.gap(x.contiguous()).squeeze().contiguous())
        else:
            ...
            for blk_idx, blk in enumerate(self.cls_attn_blocks):
                if(return_middle):
                    middle_dict[f"block_cls.{blk_idx}"] = x
                x = blk(x)
            if(return_middle):
                middle_dict[f"block_output"] = x
            x = self.norm(x)[:, 0]
        if(self.adl_loss):
            return x, attn_list
        if(return_middle):
            return x, middle_dict
        if(return_extra):
            return x, nca_middle_dict
        return x

    def forward(self, x, return_middle = False, return_extra = False):
        x = self.forward_features(x, return_middle, return_extra)
        if(self.adl_loss):
            x, attn_list = x
        if(return_middle):
            x, middle_dict = x
...
def _create_fan(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = build_model_with_cfg_old(
...
#                 'times',
#                 'energy_minimization',
#                 'weighted_scale_combine',
#                 'nca_local_perception',
#                 'nca_local_perception_loop',
#                 'energy_point_wise',
#             ]
#     for key in params:
#         if key in kwargs:
#             new_args = {key:kwargs.get(key)}
#             model_args.update(new_args)
#     print(model_args)
    backbone = _create_hybrid_backbone(pretrained=False, pretrained_strict=False, **model_args)
    model_kwargs = dict(
        patch_size=16, embed_dim=448, depth=depth, num_heads=8, eta=1.0, tokens_norm=True, sharpen_attn=False, **kwargs)
