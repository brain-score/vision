...
class FANBlock(nn.Module):
    ...
    def forward(self, x, attn=None, return_attention=False):
        ...
        if(self.nca_local_perception_model is not None):
            ...
            global_perception, _ = self.attn(x_norm, H, W)
...
class FAN(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12, sharpen_attn=False, channel_dims=None,
        ...
        if(self.nca_model):
            self.nca_drop_path1 = DropPath(0.1) # if drop_path_rate > 0. else nn.Identity()
    ...
    def forward_features(self, x, return_middle = False):
        ...
        if(self.nca_model):
            ...
            nca_output = self.nca_drop_path1(nca_output)
            x = x + nca_output
        nca_idx = 1
        ...
        if(self.nca_model):
            ...
            nca_output = self.nca_drop_path2(nca_output)
            x = x + nca_output
