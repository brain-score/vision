...
def named_apply(
    ...
    for ex_name in exclude_name:
        if ex_name in name:
            print(name, "not apply given init")
            return module
    if not depth_first and include_root:
        fn(module=module, name=name)
    for child_name, child_module in module.named_children():
        child_name = '.'.join((name, child_name)) if name else child_name
        named_apply(fn=fn, module=child_module, name=child_name, depth_first=depth_first, include_root=True, exclude_name = exclude_name)
