...
class ConVit(nn.Module):
    ...
    def __init__(
        ...
        if(nca_model):
            ...
            for i in range(self.num_nca):
                nca_drop_path = DropPath(0.1) # DropPath(dpr[self.nca_apply_layer[i]-1]) if dpr[self.nca_apply_layer[i]-1] > 0. else nn.Identity()
    ...
    def forward_features(self, x, return_extra = False):
        ...
        for u, blk in enumerate(self.blocks):
            if(self.nca_on):
                if(u in self.nca_apply_layer):
                    nca_output, nca_middle_output = self.nca[nca_idx](self.nca_norm[nca_idx](x.contiguous()).contiguous())
    ...
    def forward(self, x, return_extra = False):
        x = self.forward_features(x, return_extra)
        if(return_extra):
            x, nca_middle_dict = x
