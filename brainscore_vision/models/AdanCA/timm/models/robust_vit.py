...
class PoolingTransformer(nn.Module):
    ...
    def forward_features(self, x, return_extra = False):
        ...
        for stage in range(len(self.pools)):
            x = self.transformers[stage](x, return_extra)
            
            x = self.pools[stage](x)
        x = self.transformers[-1](x, return_extra)
        if(return_extra):
            x, nca_middle_dict = x
        cls_features = self.norm(self.gap(x).squeeze())
        if(return_extra):
            return cls_features, nca_middle_dict
        return cls_features

    def forward(self, x, return_extra = False):
        cls_features = self.forward_features(x, return_extra)
        if(return_extra):
            cls_features, nca_middle_dict = cls_features
        output = self.head(cls_features)
        if(return_extra):
            return output, nca_middle_dict
        return output
