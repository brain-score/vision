    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                qkv_bias=qkv_bias,
                qk_norm=qk_norm,
                init_values=init_values, # 1e-5 if nca_model > 0 else 
        ...
        if(nca_model):
            ...
            if(nca_model == 20):
                self.num_nca = 2
                self.nca_apply_layer = [5, 10]
            if(nca_model == 21):
                self.num_nca = 2
                self.nca_apply_layer = [5, 9]
            if(nca_model == 22):
                self.num_nca = 1
                self.nca_apply_layer = [5]
            nca_drop_path_list = []
            nca_norm_list = []
            nca_list = []
            for i in range(self.num_nca):
                nca_drop_path = DropPath(0.1) # if dpr[self.nca_apply_layer[i]-1] > 0. else nn.Identity()
                nca_norm = norm_layer(embed_dim) if before_nca_norm else nn.Identity()
                nca = NCA(
                    dim = embed_dim,
                    num_heads = num_heads,
                    act_layer=nn.GELU,
                ...
                print("additional FLOPs:", nca.flops(verbose = True) / 1e9, nca_drop_path)
                nca_list.append(nca)
                nca_drop_path_list.append(nca_drop_path)
                nca_norm_list.append(nca_norm)
            self.nca = nn.ModuleList(nca_list)
            self.nca_drop_path = nn.ModuleList(nca_drop_path_list)
            self.nca_norm = nn.ModuleList(nca_norm_list)
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if(self.nca_on):
                nca_idx = 0
                for i, blk in enumerate(self.blocks):
                    if(i in self.nca_apply_layer):
                        if(self.global_pool in ('token',)):
                            nca_input = x[:, 1:]
                            cls_token = x[:, :1]
                        else:
                            nca_input = x
                            cls_token = None
                        nca_output, nca_middle_output = self.nca[nca_idx](self.nca_norm[nca_idx](nca_input.contiguous()).contiguous())
                        nca_output = self.nca_drop_path[nca_idx](nca_output)
                        nca_output = nca_input + nca_output
                        if(self.global_pool in ('token',)):
                            x = torch.cat([cls_token, nca_output], dim = 1)
                        else:
                            x = nca_output
                        nca_idx += 1
                    x = blk(x)
