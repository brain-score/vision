...
def get_model_list():
    #return ['CornetVanilla2', 'CornetCustom2']
    return ['CornetVanilla2']

def get_model(model_name):
    model = CORnet_S()
    model = torch.nn.DataParallel(model)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # if model_name == 'CornetVanilla':
    #     url = 'https://s3.amazonaws.com/cornet-models/cornet_s-1d3f7974.pth'
    # elif model_name == 'CornetCustom':
    #     url = 'https://www.dropbox.com/s/xcck6czcfpne8li/checkpoint.pth?dl=1'
    # else:
    #     print(f'{model_name} is no a valid model name.')
    #     raise
    ...
    wrapper = TemporalPytorchWrapper(identifier=model_name, model=model, preprocessing=preprocessing)
