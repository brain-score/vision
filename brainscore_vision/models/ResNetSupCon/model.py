...
download_file("supcon.pth", "supcon.pth")


ckpt = torch.load("supcon.pth",  map_location='cpu')
...
model_ft = SupConResNet(name = 'resnet50')
model_ft.load_state_dict(new_state_dict)
layers_ = [layer for layer, _ in model_ft.named_modules()][1:]
layers = [k for k in layers_ if 'conv3' in k]
layers.extend(layers_[-4:])
layers.append('encoder.conv1')
model_ft = model_ft.to(device)
#print(layers)
# get an activations model from the Pytorch Wrapper
activations_model = PytorchWrapper(identifier='ResNetConSup', model= model_ft , preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='ResNetConSup', activations_model=activations_model,
