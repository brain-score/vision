...
def get_model_list():
    return ['Zejin_Lu_dvd_b_imagenet_resnet50']

def get_model(name):
    assert name == 'Zejin_Lu_dvd_b_imagenet_resnet50'

    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_784/",
                      relative_path="dvd_b_checkpoint_best.pth",
                      version_id="pVae_ANsSS8HNwzvCrGqtvuOrZ9.Y.Gs",
                      ),
    ...
    wrapper = PytorchWrapper(identifier='Zejin_Lu_dvd_b_imagenet_resnet50', model=model, preprocessing=preprocessing)
