...
def get_model_list():
    return ['Zejin_Lu_dvd_baseline_imagenet_resnet50']

def get_model(name):
    assert name == 'Zejin_Lu_dvd_baseline_imagenet_resnet50'

    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_784/",
                      relative_path="dvd_baseline_checkpoint_best.pth",
                      version_id="60x_AyvgeCC1dLaX_rwchzWLwQeE4vlc",
                      ),
    ...
    replace = ['module._orig_mod.', 'module.']
    new_state_dict = strip_prefix(state_dict, replace)
    ...
    wrapper = PytorchWrapper(identifier='Zejin_Lu_dvd_baseline_imagenet_resnet50', model=model, preprocessing=preprocessing)
