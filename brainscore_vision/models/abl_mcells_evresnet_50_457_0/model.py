...
def get_model(name):
    assert name == 'abl_mcells_evresnet_50_457_0'
    model = EVNet(
        with_retinablock=True, with_voneblock=True, model_arch='resnet50',
        image_size=224, visual_degrees=7, num_classes=1000, k_exc= 3.14,
        colors_p_cells=['r/g', 'g/r', 'b/y'], p_channels=3, m_channels=0, retinal_noise_mode=None,
        noise_mode=None, gabor_seed=457, sf_max=9, stride=4, gabor_color_prob=[.34, .34, .32],
        )
    ...
    wrapper = PytorchWrapper(
        identifier='abl_mcells_evresnet_50_457_0',
