...
def get_model_list(network, keyword, iteration):
    return [f"alexnet_ambient_iteration=2"]

def download_checkpoint(network, keyword, iteration):
    url = f"https://eggerbernhard.ch/shreya/latest_alexnet/ambient_2.ckpt"
    output = f"alexnet_ambient_iteration=2.ckpt"
...
def get_model(network, keyword, iteration):
    ...
    activations_model = PytorchWrapper(identifier=f"alexnet_ambient_iteration=2",
                                       model=model,
...
if __name__ == '__main__':
    ...
    iteration = f"2"  # Example iteration
