...
def get_model_list():
    return ['alexnet_less_variation_iteration=2']

def get_model(name):
    keyword = 'less_variation'
    iteration = 2
    network = 'alexnet'
    url = 'https://eggerbernhard.ch/shreya/latest_alexnet/less_variation_2.ckpt'
    output = 'alexnet_less_variation_iteration=2.ckpt'
    gdown.download(url, output)


    if keyword != 'imagenet_trained' and keyword != 'no_training':
        lx_whole = [f"alexnet_less_variation_iteration=2.ckpt"]
        if len(lx_whole) > 1:
            lx_whole = [lx_whole[-1]]
    ...
    assert name == 'alexnet_less_variation_iteration=2'
    url = 'https://eggerbernhard.ch/shreya/latest_alexnet/less_variation_2.ckpt'
    output = 'alexnet_less_variation_iteration=2.ckpt'
    gdown.download(url, output)
    layers = []
