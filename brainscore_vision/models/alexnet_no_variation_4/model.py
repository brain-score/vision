...
def get_model_list():
    return ['alexnet_no_variation_iteration=4']

def get_model(name):
    keyword = 'no_variation'
    iteration = 4
    network = 'alexnet'
    url = 'https://eggerbernhard.ch/shreya/latest_alexnet/no_variation_4.ckpt'
    output = 'alexnet_no_variation_iteration=4.ckpt'
    gdown.download(url, output)


    if keyword != 'imagenet_trained' and keyword != 'no_training':
        lx_whole = [f"alexnet_no_variation_iteration=4.ckpt"]
        if len(lx_whole) > 1:
            lx_whole = [lx_whole[-1]]
    ...
    assert name == 'alexnet_no_variation_iteration=4'
    url = 'https://eggerbernhard.ch/shreya/latest_alexnet/no_variation_4.ckpt'
    output = 'alexnet_no_variation_iteration=4.ckpt'
    gdown.download(url, output)
    layers = []
