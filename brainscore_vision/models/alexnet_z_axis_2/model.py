...
def get_model_list():
    return ['alexnet_z_axis_iteration=2']

def get_model(name):
    keyword = 'z_axis'
    iteration = 2
    network = 'alexnet'
    url = 'https://eggerbernhard.ch/shreya/latest_alexnet/z_axis_2.ckpt'
    output = 'alexnet_z_axis_iteration=2.ckpt'
    gdown.download(url, output)


    if keyword != 'imagenet_trained' and keyword != 'no_training':
        lx_whole = [f"alexnet_z_axis_iteration=2.ckpt"]
        if len(lx_whole) > 1:
            lx_whole = [lx_whole[-1]]
    ...
    assert name == 'alexnet_z_axis_iteration=2'
    url = 'https://eggerbernhard.ch/shreya/latest_alexnet/z_axis_2.ckpt'
    output = 'alexnet_z_axis_iteration=2.ckpt'
    gdown.download(url, output)
    layers = []
