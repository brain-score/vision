...
def get_model_list():
    return ['alexnet_z_axis_iteration=3']

def get_model(name):
    keyword = 'z_axis'
    iteration = 3
    network = 'alexnet'
    url = 'https://eggerbernhard.ch/shreya/latest_alexnet/z_axis_3.ckpt'
    output = 'alexnet_z_axis_iteration=3.ckpt'
    gdown.download(url, output)


    if keyword != 'imagenet_trained' and keyword != 'no_training':
        lx_whole = [f"alexnet_z_axis_iteration=3.ckpt"]
        if len(lx_whole) > 1:
            lx_whole = [lx_whole[-1]]
    ...
    assert name == 'alexnet_z_axis_iteration=3'
    url = 'https://eggerbernhard.ch/shreya/latest_alexnet/z_axis_3.ckpt'
    output = 'alexnet_z_axis_iteration=3.ckpt'
    gdown.download(url, output)
    layers = []
