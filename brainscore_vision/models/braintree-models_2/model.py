...
def get_model_list():
    ...
    models += ['CORnet-S']
    models += ['BT_CORnet-S-100', 'BT_CORnet-S-1001', 'BT_CORnet-S-4004']

    return models

def CORnet_model(model_id):
    import cornet
    from model_tools.activations.pytorch import load_preprocess_images
    preprocessing = functools.partial(load_preprocess_images, image_size=224)
    if model_id == 'CORnet-S':
        model = getattr(cornet, f'cornet_s')
        model = model(pretrained=True, map_location=torch.device('cpu'))
    if 'BT_CORnet-S' in model_id:
        model = getattr(cornet, f'cornet_s')
        model = model(pretrained=True, map_location=torch.device('cpu'))
        ep = model_id.split('-')[-1]
        #weights = torch.load('./models/weights/epoch_01.pth.tar', map_location=torch.device('cpu'))
        weights = torch.load(
            os.path.join(os.path.dirname(__file__), f'weights/epoch_{ep}.pth.tar'), 
            map_location=torch.device('cpu')
        )
        model.load_state_dict(weights['state_dict'])
    model = model.module
    wrapper = PytorchWrapper(model=model, preprocessing=preprocessing, identifier=model_id)
