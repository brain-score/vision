...
#         # Following the Hugging Face preprocessing approach
#         images = [processor(images=image, return_tensors="pt", **kwargs)['pixel_values'] for image in images]
#         images = torch.cat(images).cpu().numpy()
#     return images

# # Helper function for preprocessing (aligning with Hugging Face example)
# def preprocess_image(im: np.ndarray, model_input_size: list) -> torch.Tensor:
#     if len(im.shape) < 3:
#         im = im[:, :, np.newaxis]  # Add channel dimension if missing
#     im_tensor = torch.tensor(im, dtype=torch.float32).permute(2, 0, 1)  # Convert to CHW format
#     im_tensor = F.interpolate(torch.unsqueeze(im_tensor, 0), size=model_input_size, mode='bilinear')  # Resize
#     image = torch.divide(im_tensor, 255.0)  # Normalize to [0, 1]
#     image = np.normalize(image, [0.5, 0.5, 0.5], [1.0, 1.0, 1.0])  # Normalize to [0.5, 0.5, 0.5] mean/std
#     return image
