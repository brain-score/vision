...
from brainscore_vision.models.chresmax_v3_2.local_timm.models.RESMAX import chresmax_v3_2

def load_model(device=None):
    if device is None:
        device = 'cuda' if torch.cuda.is_available() else 'cpu'

    checkpoint_url = "https://huggingface.co/cmulliken/chresmax_v3_2/resolve/main/model_best.pth.tar"
    args_url = "https://huggingface.co/cmulliken/chresmax_v3_2/resolve/main/args.yaml"
...
def get_model(name):
    assert name == 'chresmax_v3_2'
    model = load_model()
    preprocessing = functools.partial(load_preprocess_images, image_size=322)
    wrapper = PytorchWrapper(identifier='chresmax_v3_2', model=model, preprocessing=preprocessing)
