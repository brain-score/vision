...
def load_model(model_name,
    ...
    # Extract numeric parameters from model_name robustly
    import re
    channel_match = re.search(r'ip_(\d+)_', model_name)
    channel_size = int(channel_match.group(1)) if channel_match else 3
    
    ip_match = re.search(r'_ip_(\d+)_', model_name)
    ip = int(ip_match.group(1)) if ip_match else 3

    c1_match = re.search(r'_c1\[(\d+)\]', model_name)
    classifier_input_size = int(c1_match.group(1)) if c1_match else 1000

    print(f"Loading model: {model_name} from Hugging Face with channel size {channel_size}, ip {ip}, classifier input size {classifier_input_size}")

    checkpoint_url = "https://huggingface.co/cmulliken/chresmax_test/resolve/main/model_best.pth.tar"
...
def get_model(name):
    assert name == 'chresmax_v3_cl_01_v4'
    ...
    wrapper = PytorchWrapper(identifier='chresmax_v3_cl_01_v4', model=model, preprocessing=preprocessing)
