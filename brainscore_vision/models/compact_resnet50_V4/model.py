...
class CompactModel(nn.Module):
    ...
    def forward(self, x):
        ...
        for ichannel in range(3):  # recenter
            x[:,ichannel,:,:] = x[:,ichannel,:,:] - rgb_vals[ichannel]
...
def get_model_list():
    return ['compact_resnet50_V4']


def get_model(name):
    assert name == 'compact_resnet50_V4'

    pytorch_device = torch.device('cpu')


    # basedir = os.path.dirname(os.path.abspath(__file__))
    # load_path = os.path.join(basedir, 'compact_ResNet50_V4_50filters.pt')
    # load_path = './brainscore_vision/models/compact_resnet50_V4/ResNet50_V4_50filters.pt'
...
# https://brainscore-storage.s3.us-east-2.amazonaws.com/brainscore-vision/models/user_718/compact_ResNet50_V4_50filters.pt?versionId=oa0hxVd5Yak8BhbJBEiZdYmzOm1jlbnM
    ...
    weights_path = load_weight_file(bucket="brainscore-storage", folder_name="brainscore-vision/models",
                                    relative_path="user_718/compact_ResNet50_V4_50filters.pt",
                                    version_id="oa0hxVd5Yak8BhbJBEiZdYmzOm1jlbnM")
    state_dict = torch.load(weights_path, map_location=lambda storage, loc: storage)  # map onto cpu
    ...
    # activations_model = PytorchWrapper(identifier='compact_resnet50_V4', model=pytorch_model, preprocessing=preprocessing)
    # wrapper = ModelCommitment(identifier='compact_resnet50_V4', activations_model=activations_model,
    #                         # specify layers to consider
    #                         layers=['layer1_act', 'layer2_act', 'layer3_act', 'layer4_act'])
