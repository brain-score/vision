...
download_file("checkpoint_params_skgrcnn55.pt", "checkpoint_params_skgrcnn55.pt")

#model.load_state_dict(torch.load(dir_path + "/my_model.pth"))
model_ft = skgrcnn55() #models.resnet50(pretrained=True)
model_ft.load_state_dict(torch.load("checkpoint_params_skgrcnn55.pt"))
model_ft = model_ft.to(device)


# get an activations model from the Pytorch Wrapper
activations_model = PytorchWrapper(identifier='skgcrnn', model= model_ft , preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='skgcrnn', activations_model=activations_model,
                        # specify layers to consider
                        layers=['layer1.d_conv_1e', 'layer2.d_conv_1e', 'layer3.d_conv_1e', 'layer4.iter_g_3.1', 'lastact.1', 'classifier'])
...
def get_layers(name):
    ...
    return  ['layer1.d_conv_1e', 'layer2.d_conv_1e', 'layer3.d_conv_1e', 'layer4.iter_g_3.1', 'lastact.1', 'classifier']
