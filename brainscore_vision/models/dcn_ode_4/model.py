...
model.load_state_dict(torch.load(os.path.join(os.path.dirname(os.path.realpath(__file__)),'dcn_ode.pth'), map_location = device)['state_dict'])
model = model.module  #Remove DataParallel Wrapper
model.fc_linear = nn.Linear(128,1000) #Imagenet num_classes
print(model)
activations_model = PytorchWrapper(identifier='dcn_ode_apr19_tinyImagenet', model=model, preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='dcn_ode_apr19_tinyImagenet', activations_model=activations_model,
...
def get_model_list():
    ...
    return ['dcn_ode_apr19_tinyImagenet']
...
def get_model(name):
    ...
    """
    assert name == 'dcn_ode_apr19_tinyImagenet'
