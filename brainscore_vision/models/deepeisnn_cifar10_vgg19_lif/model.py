"""Brain-Score wrapper for DeepEISNN EI-SNN CIFAR10 VGG19."""
...
MODEL_IDENTIFIER = "deepeisnn_cifar10_vgg19_lif"
HF_REPO_ID = "vwOvOwv/DeepEISNN"
HF_SUBFOLDER = "CIFAR10-VGG19"
IMAGE_SIZE = 32
BATCH_SIZE = 64
SEED = 2025
...
def _collect_lif_layer_names(model: torch.nn.Module) -> list[str]:
    return [
        name
...
def _infer_temporal_steps(model: torch.nn.Module) -> int:
    if hasattr(model, "T"):
        return int(model.T)
    for attr in ("init_expand", "final_split", "split1"):
        module = getattr(model, attr, None)
        if module is not None and hasattr(module, "T"):
            return int(module.T)
    return 1
...
class ResettingSNNPytorchWrapper:
    def __init__(self, model, preprocessing, identifier, batch_size):
        ...
        self._temporal_steps = _infer_temporal_steps(model)
...
def get_layers(name):
    ...
    if _LAYER_NAMES is None:
        _LAYER_NAMES = _collect_lif_layer_names(_get_raw_model())
    return _LAYER_NAMES
...
def get_behavioral_readout_layer(name):
    ...
    output_layers = [
        layer_name
        for layer_name, module in _get_raw_model().named_modules()
        if isinstance(module, SpikingEiNorm1d) and module.output_layer
    ]
    if not output_layers:
        raise RuntimeError("No output SpikingEiNorm1d layer found.")
...
if __name__ == "__main__":
    ...
    print(_collect_lif_layer_names(model))
