...
class SpikingResNet(nn.Module):
    ...
    def __init__(self, T: int, num_layers: int, in_channels: int, n_outputs: int,
                 ...
                 seq_input: bool = False, imagenet_backbone: bool = False):
        """Initialize model.
        ...
        Args:
            ...
            imagenet_backbone: Whether to use the 7x7/stride-2 ImageNet stem.
        """
        super().__init__()
        self.in_channels = 64
        stem_kernel_size = 7 if imagenet_backbone else 3
        stem_stride = 2 if imagenet_backbone else 1
        stem_padding = 3 if imagenet_backbone else 1
        ...
        self.maxpool = (
            ...
            if imagenet_backbone else nn.Identity()
        )
...
class SpikingEiResNet(nn.Module):
    ...
    def __init__(self, T: int, num_layers: int, in_channels: int, n_outputs: int,
        ...
        self.norm1 = SpikingEiNorm2d(
            self.in_channels,
            in_channels * stem_kernel_size * stem_kernel_size,
            ei_ratio,
            device=device,
        )
        self.split1 = SplitTemporalDim(T)
