...
class CNN_block(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dim_hook):
...
class DFA_Network(nn.Module):
    def __init__(self):
        ...
        # ReLU activation inside conv_block
        ...
        # - kernel size = 5 - standard kernel size
        self.conv2 = CNN_block(6, 12, 5, dim_hook = [10,12,10,10])
        # ReLU activation inside conv_block
        ...
        self.fully_connected2 = FC_block(120, 10, dim_hook = [10,10])

    def forward(self, x, labels):
        x = self.conv1(x, labels, self.y)
        # Relu activation inside conv block
        x = self.pool(x)
...
activations_model = PytorchWrapper(identifier='cb-direct-feedback-alignment', model=DFA_Network(), preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='cb-direct-feedback-alignment', activations_model=activations_model,
                        # specify layers to consider
                        layers=['conv1', 'conv2', 'fully_connected1'])
...
def get_model_list():
    ...
    return ['cb-direct-feedback-alignment']
...
def get_model(name):
    ...
    """
    assert name == 'cb-direct-feedback-alignment'
