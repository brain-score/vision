...
class Dorinet_CORnet_RT(nn.Module):
...
def get_model_list():
    return ['Dorinet_CORnet_RT']


def get_model(name):
    assert name == 'Dorinet_CORnet_RT'
    model = Dorinet_CORnet_RT()
    url = 'https://s3.amazonaws.com/cornet-models/cornet_rt-933c001c.pth'
    torch.hub.load_state_dict_from_url(url,map_location=torch.device('cuda'))
    preprocessing = functools.partial(load_preprocess_images, image_size=224)
    wrapper = PytorchWrapper(identifier='Dorinet_CORnet_RT', model=model, preprocessing=preprocessing)
...
__all__ = ['dorinet_CORnet_RT', 'Dorinet_CORnet_RT']

model_urls = {
    'Dorinet_CORnet_RT': 'https://download.pytorch.org/models/alexnet-owt-4df8aa71.pth',
}