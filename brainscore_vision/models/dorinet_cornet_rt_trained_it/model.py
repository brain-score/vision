...
class Dorinet_CORnet_RT_IT(nn.Module):
...
def get_model_list():
    return ['Dorinet_CORnet_RT_IT']


def get_model(name):
    assert name == 'Dorinet_CORnet_RT_IT'
    model = Dorinet_CORnet_RT_IT()
    ...
    wrapper = PytorchWrapper(identifier='Dorinet_CORnet_RT_IT', model=model, preprocessing=preprocessing)
...
__all__ = ['dorinet_CORnet_RT_IT', 'Dorinet_CORnet_RT_IT']

model_urls = {
    'Dorinet_CORnet_RT_IT': 'https://download.pytorch.org/models/alexnet-owt-4df8aa71.pth',
}