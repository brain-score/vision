...
def get_model_list():
    return ['effnetb1_vonemap_retrain']
...
def get_model(name):
    assert name == 'effnetb1_vonemap_retrain'
    model= EffNetVOneMaps()
    dir_path = os.path.dirname(os.path.realpath(__file__))
    model.load_state_dict(torch.load(dir_path + "/effnetb1_vonemaps_epoch2_score0.48412793121796205_best.pth", map_location=torch.device('cpu'))["model"])
