...
def get_model_list():
    return ['effnetb1_cutmixpatch_augmix_robust32_e0_324x288']
...
def get_model(name):
    assert name == 'effnetb1_cutmixpatch_augmix_robust32_e0_324x288'
    model_tf_efficientnet_b1_ns= EffNetBX()
    dir_path = os.path.dirname(os.path.realpath(__file__))
    model_tf_efficientnet_b1_ns.load_state_dict(torch.load(dir_path + "/tf_efficientnet_b1_ns_robust_cutmixpatchresize_augmix_epoch0_score0.46524872477166135_best.pth", map_location=torch.device('cpu'))["model"])
    ...
    for n, m in model.named_modules():
      if isinstance(m, nn.BatchNorm2d) and any(x in n for x in ["conv_stem", "blocks.0", "blocks.1", "blocks.2", "blocks.3.0", "blocks.3.1", "blocks.3.2"]) or n =="bn1":
        print(f"Freeze {n, m}")
        m.eval()
