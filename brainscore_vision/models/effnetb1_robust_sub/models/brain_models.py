...
def get_model_list():
    return ['effnetb1_cutmixpatch_augmix_robust32_avge4e7_LayersSelected_TTA_324x288']
...
class TTAModelCommitment(ModelCommitment):
    ...
    def __init__(self, identifier, activations_model, layers, visual_degrees=8):
            ...
            super(TTAModelCommitment, self).__init__(identifier=identifier, activations_model=activations_model,
                                                            layers=layers, region_layer_map=None,
                                                            visual_degrees=visual_degrees)      
    def look_at(self, stimuli, number_of_trials=1):
        stimuli_identifier = stimuli.identifier
        for trial_number in range(number_of_trials):
            print(f"[TTACOMMIT] TRIAL {stimuli.identifier} NUMBER {trial_number}/{number_of_trials}")
...
class StochasticLayerMappedModel(LayerMappedModel):
    def run_activations(self, stimuli, layers, number_of_trials=1):
        stimuli_identifier = stimuli.identifier
        for trial_number in range(number_of_trials):
            print(f"[StochasticLayer] TRIAL {stimuli.identifier} NUMBER {trial_number}/{number_of_trials}")
            if stimuli_identifier:
                stimuli.identifier = stimuli_identifier + '-trial' + f'{trial_number:03d}'
            if trial_number == 0:
...
def get_model(name):
    assert name == 'effnetb1_cutmixpatch_augmix_robust32_avge4e7_LayersSelected_TTA_324x288'
    ...
    activations_model  = TTAPytorchWrapper(identifier='effnetb1_cutmixpatch_augmix_robust32_avge4e7_LayersSelected_TTA_324x288', model=model, preprocessing=preprocessing, batch_size=8)
    model = TTAModelCommitment(identifier='effnetb1_cutmixpatch_augmix_robust32_avge4e7_LayersSelected_TTA_324x288', activations_model=activations_model ,
                        # specify layers to consider
                        layers=layers)
    model.layer_model.region_layer_map['V1'] = 'blocks.3.0'
    model.layer_model.region_layer_map['V2'] = 'blocks.3.2'
    model.layer_model.region_layer_map['V4'] = 'blocks.4.0.conv_pwl'
    model.layer_model.region_layer_map['IT'] = 'blocks.5.0.conv_pwl'
