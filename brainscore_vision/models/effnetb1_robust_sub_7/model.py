...
def get_model_list():
    return ['effnetb1_cutmixrespatch_SAM_robust32_e10_manylayers_324x288']
...
def get_model(name):
    assert name == 'effnetb1_cutmixrespatch_SAM_robust32_e10_manylayers_324x288'
    model_tf_efficientnet_b1_ns= EffNetBX()
    dir_path = os.path.dirname(os.path.realpath(__file__))
    model_tf_efficientnet_b1_ns.load_state_dict(torch.load(dir_path + "/tf_efficientnet_b1_ns_robust_linf_e2_cutmixresizepatch_SAM_epoch10_score0.5047809230913052_best.pth", map_location=torch.device('cpu'))["model"])
