...
def get_model_list():
    return ['effnetb1_cutmix_augmix_sam_epoch2_324x288']
...
def get_model(name):
    assert name == 'effnetb1_cutmix_augmix_sam_epoch2_324x288'
    model_tf_efficientnet_b1_ns= EffNetBX()
    dir_path = os.path.dirname(os.path.realpath(__file__))
    model_tf_efficientnet_b1_ns.load_state_dict(torch.load(dir_path + "/tf_efficientnet_b1_ns_cutmix_augmix_SAM_epoch2_score0.514767142402596_best.pth", map_location=torch.device('cpu'))["model"])
...
def get_layers(name):
    ...
    'blocks.6', 'blocks.6.0', 'blocks.6.1', 'global_pool', 'global_pool.flatten', 'global_pool.pool', 'classifier']
