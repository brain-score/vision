...
def get_model_list():
    return ['effnetb2_cutmixpatch_robust35_avgee5e3_manylayers_424x377']
...
def get_model(name):
    assert name == 'effnetb2_cutmixpatch_robust35_avgee5e3_manylayers_424x377'
    model_tf_efficientnet_b1_ns= EffNetBX()
    dir_path = os.path.dirname(os.path.realpath(__file__))
    model_tf_efficientnet_b1_ns.load_state_dict(torch.load(dir_path + "/tf_efficientnet_b2_ns_robust_cutmixpatchresize_e3e5.pth", map_location=torch.device('cpu'))["model"])
