...
def get_model_list():
    return ['effnetb2_cutmix_augmix_epoch4_352x313']

class EffNetBX(nn.Module):
    def __init__(self,):
        super().__init__ ()
        self.efnet_model = timm.create_model('tf_efficientnet_b2_ns', pretrained=True)
...
def get_model(name):
    assert name == 'effnetb2_cutmix_augmix_epoch4_352x313'
    dir_path = os.path.dirname(os.path.realpath(__file__))
    model= EffNetBX()
    model.load_state_dict(torch.load(dir_path + "/tf_efficientnet_b2_ns_cutmix_augmix_epoch2_score0.5179265011008524_best.pth", map_location=torch.device('cpu'))["model"])
...
def get_layers(name):
 ...
 'blocks.4.0', 'blocks.4.1', 'blocks.4.2', 'blocks.4.3',
 'blocks.5.0', 'blocks.5.1', 'blocks.5.2', 'blocks.5.3', 'blocks.5.4',
 'blocks.6.0', 'blocks.6.1',
 "conv_head", "act2", "global_pool", "global_pool.flatten", "global_pool.pool"]
