...
def get_model_list():
    return ['effnetb0_cutmix_augmix_epoch1']
...
def get_model(name):
    assert name == 'effnetb0_cutmix_augmix_epoch1'
    model_tf_efficientnet_b0_ns= EffNetBX()
    dir_path = os.path.dirname(os.path.realpath(__file__))
    model_tf_efficientnet_b0_ns.load_state_dict(torch.load(dir_path + "/tf_efficientnet_b0_ns_cutmix_augmix_epoch1_score0.5112402985466626_best.pth", map_location=torch.device('cpu'))["model"])
