...
def reload_weights(model_path, model, reload_model, device):
    if reload_model:
        ...
        # Use strict=False to allow loading weights even when new modules (like logits)
        # don't have corresponding saved weights
...
class Dummy(nn.Module):
    """
    This class is only here to receive a hook on the value out with the proper shape.
    Reshapes 4D patch-based output to 2D by flattening spatial dimensions.
    """
    def __init__(self, flatten_patches=True):
        super(Dummy, self).__init__()
        self.flatten_patches = flatten_patches

    def forward(self, x):
        # x has shape (batch, channels, n_patches_y, n_patches_x)
        # Flatten to (batch, channels * n_patches_y * n_patches_x) if enabled
        if self.flatten_patches and x.ndim > 2:
            batch_size = x.shape[0]
            return x.view(batch_size, -1)  # Returns (batch, channels * patches_y * patches_x)
        return x


class LogitsWrapper(nn.Module):
    """
    Wrapper module that applies a classifier to the encoder output.
    This allows Brain-Score to extract 'logits' via hooks.
    """
    def __init__(self, encoder_output_dim=1024, num_classes=1000):
        super(LogitsWrapper, self).__init__()
    ...
    def forward(self, x):
        # x is (batch, features) from the final encoder layer
        # After mean over patches: (batch, 1024)
...
class FullVisionModel(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        # Add logits wrapper for ImageNet classification
        # This will be used when Brain-Score requests 'logits' layer
        self.logits = LogitsWrapper(encoder_output_dim=1024, num_classes=1000)
    ...
    def forward(self, x, is_normalized=False, all_layers=False, keep_patches=False):
        ...
        # Pass final encoder output through logits classifier
        # This ensures Brain-Score can extract 'logits' via hooks on self.logits
        final_output = outs[-1]
        _ = self.logits(final_output)  # Call logits to trigger any hooks
