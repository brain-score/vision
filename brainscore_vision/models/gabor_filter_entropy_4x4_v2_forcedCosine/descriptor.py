...
class PositiveNegativeGlobalGaborFilterEntropyExtractor(FoveatedGaborFilterEntropyExtractor):
    """Foveated entropy plus a global-minus-local channel.

    For each Gabor filter, the global entropy is repeated across all spatial
    windows. The returned feature vector is [local, alpha * global - local].
    Adding a constant offset happens in model.py only to satisfy benchmarks
    that screen units by absolute activation level; it does not alter Pearson
    correlation geometry.
    """

    def __init__(self, config: GaborEntropyConfig | None = None, alpha: float = 0.5):
        self.alpha = alpha
        super().__init__(config or GaborEntropyConfig())
    ...
    def extract_one(self, image: np.ndarray | Image.Image) -> np.ndarray:
        ...
        for current_filter in np.moveaxis(self.filter_bank, 2, 0):
            ...
            local_vector = self._block_entropy_vector(filtered)
            local_vectors.append(local_vector)
            global_values.append(image_entropy(filtered, self.config.entropy_bins))

        local = np.concatenate(local_vectors).astype(np.float64, copy=False)
        n_windows = len(local_vectors[0]) if local_vectors else 0
        global_broadcast = np.repeat(np.asarray(global_values, dtype=np.float64), n_windows)
        descriptor = np.concatenate([local, self.alpha * global_broadcast - local])
        return descriptor.astype(self.config.output_dtype, copy=False)
