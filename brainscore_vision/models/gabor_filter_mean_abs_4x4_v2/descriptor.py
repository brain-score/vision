...
class MeanAbsFoveatedGaborExtractor(GaborFilterEntropyExtractor):
    """Foveated Gabor response magnitude control matched to gabor_filter_entropy_4x4_v2."""
    ...
    def extract_one(self, image: np.ndarray | Image.Image) -> np.ndarray:
        ...
        vectors = []
        # Entropy-modulated comparison path, intentionally inactive in this mean-abs control:
        ...
        for current_filter in np.moveaxis(self.filter_bank, 2, 0):
            ...
            vectors.append(self._block_mean_abs_vector(filtered))
            # normalized = self._normalize_zero_one(filtered)
            # vectors.append(self._block_modulated_vector(filtered, normalized, max_entropy))

        descriptor = np.concatenate(vectors)
        return descriptor.astype(self.config.output_dtype, copy=False)

    def _block_mean_abs_vector(self, filtered: np.ndarray) -> np.ndarray:
        image_size = filtered.shape[0]
        base_window = self.mid_window
        starts = range(0, image_size - base_window + 1, self.base_stride)
        image_center = (image_size - 1) / 2.0
        ...
        for row_start in starts:
            ...
            for col_start in starts:
                ...
                window_size = self._window_size_for_eccentricity(eccentricity)
                row_slice, col_slice = self._window_slices(base_row_center, base_col_center, window_size, image_size)
                values.append(float(np.mean(np.abs(filtered[row_slice, col_slice]))))
        return np.asarray(values, dtype=np.float64)

    def _block_modulated_vector(self, filtered: np.ndarray, normalized: np.ndarray, max_entropy: float) -> np.ndarray:
        """Inactive entropy-modulated comparison kept here to make the control transparent."""
        image_size = filtered.shape[0]
        base_window = self.mid_window
        starts = range(0, image_size - base_window + 1, self.base_stride)
        image_center = (image_size - 1) / 2.0
        ...
        for row_start in starts:
            ...
            for col_start in starts:
                ...
                window_size = self._window_size_for_eccentricity(eccentricity)
                row_slice, col_slice = self._window_slices(base_row_center, base_col_center, window_size, image_size)
                response = float(np.mean(np.abs(filtered[row_slice, col_slice])))
                entropy = image_entropy(normalized[row_slice, col_slice], self.config.entropy_bins)
                entropy_weight = entropy / max_entropy if max_entropy > 0 else 0.0
                values.append(response * entropy_weight)
        return np.asarray(values, dtype=np.float64)
