...
download_file("checkpoint_params_grcnn109.pt", "checkpoint_params_grcnn109.pt")

#model.load_state_dict(torch.load(dir_path + "/my_model.pth"))
model_ft = grcnn109() #models.resnet50(pretrained=True)
model_ft.load_state_dict(torch.load("checkpoint_params_grcnn109.pt"))
...
activations_model = PytorchWrapper(identifier='grcnn_109', model= model_ft , preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='gcrnn_109', activations_model=activations_model,
