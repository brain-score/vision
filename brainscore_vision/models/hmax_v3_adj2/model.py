...
def _wrap_module_outputs(model):
    def _safe_forward_hook(mod, inp, out):
        if isinstance(out, list):
            ...
            merged = torch.stack(pooled, dim=0).mean(dim=0)  # [B,C,H,W]
            return merged
        return out

    for name, module in model.named_modules():
        if any(cls in str(type(module)) for cls in ["S1_Res", "S2_Res", "S3_Res", "S2b_Res", "C_adp"]):
...
def get_model(name):
    ...
    model = load_model()
    model = _wrap_module_outputs(model)
