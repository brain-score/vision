...
MODEL_NAME = 'lagani-trained-timagenet-FULL-SGD'
...
class Net(nn.Module):
	...
	def __init__(self, input_shape=INPUT_SHAPE):
		...
		self.fc6 = nn.Linear(1024, NUM_CLASSES) # 300-dimensional input, 10-dimensional output (one per class)
...
# we change fc6 to work with NUM_CLASSES=1000 dimensions instead of tiny imagenet (200)
state_dict['fc6.weight'] = torch.zeros([NUM_CLASSES, 1024])
state_dict['fc6.bias'] = torch.zeros([NUM_CLASSES])
