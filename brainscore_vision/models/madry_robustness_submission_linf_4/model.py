...
def get_model(name):
    assert name == 'advmad_linf_4'

    # l2 = 3
    model = MadryRobust(InputNormalize(new_mean_robust_madry , new_std_robust_madry) , 
                torchmodels.resnet50(pretrained=False))    

    path_previous_checkpoint = os.environ['HOME']+'/work_dir/madry_robustness_submission_linf_4/models/imagenet_linf_4.pt'
    ...
    wrapper = PytorchWrapper(identifier='advmad_linf_4', model=model, preprocessing=preprocessing)
