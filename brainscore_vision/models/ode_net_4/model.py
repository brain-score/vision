...
model.load_state_dict(torch.load(os.path.join(os.path.dirname(os.path.realpath(__file__)),'ode_net.pth'), map_location = device)['state_dict'])
model = model.module  #Remove DataParallel Wrapper
model.fc_linear = nn.Linear(128,1000) #Imagenet num_classes
print(model)
activations_model = PytorchWrapper(identifier='ode_net_apr19_tinyImagenet', model=model, preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='ode_net_apr19_tinyImagenet', activations_model=activations_model,
                        # specify layers to consider
                        layers=['initialConv', 'feature_layer1.odefunc.conv2', 'DS1_conv', 'feature_layer2.odefunc.conv2', 'DS2_conv', 'feature_layer3.odefunc.conv2'])
...
def get_model_list():
    ...
    return ['ode_net_apr19_tinyImagenet']
...
def get_model(name):
    ...
    """
    assert name == 'ode_net_apr19_tinyImagenet'
...
def get_layers(name):
    ...
    return ['initialConv', 'feature_layer1.odefunc.conv2', 'DS1_conv', 'feature_layer2.odefunc.conv2', 'DS2_conv', 'feature_layer3.odefunc.conv2']
