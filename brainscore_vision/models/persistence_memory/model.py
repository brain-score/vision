...
MODEL_ID = "persistence_memory"
WEIGHTS_URL = "https://huggingface.co/emirhaninan814/persistence_mem_weights/resolve/main/persistence_memory.pth"
...
def get_model(name):
    ...
    checkpoint_path = os.path.join(os.path.dirname(__file__), "persistence_memory.pth")
    if os.path.exists(checkpoint_path):
        ...
        checkpoint = torch.hub.load_state_dict_from_url(
            ...
            map_location="cpu",
            progress=False,
            file_name="persistence_memory.pth",
        )
    else:
        raise RuntimeError(
            "persistence_memory weights are not bundled because Brain-Score website "
            "uploads are limited to 50MB. Set WEIGHTS_URL to the externally hosted "
            "persistence_memory.pth URL before submitting."
        )
