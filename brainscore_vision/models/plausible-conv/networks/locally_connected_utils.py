...
def hebbian_backward_hook(module, grad_inputs, grad_outputs):
    with torch.cuda.amp.autocast(enabled=False):
        modified_grad_w = grad_inputs[-1] + module.hebb_lr * \
                          torch.einsum('bgihwkc,bgjhw->igjhwkc', module.get_b().float(), module.output.float()) \
                          / module.output.shape[0]
...
class Centered1x1LC(nn.Module):
    ...
    def forward(self, x, b, weights):
        ...
        # self.output = self.output - self.output.mean(dim=(-2, -1), keepdim=True)
            ...
            self.output = torch.einsum('bgihwkc,igjhwkc->bgjhw', x, weights).contiguous()
        return self.output
...
def tuple_into_int(x):
    if isinstance(x, int):
        return x
    return x[0]


class CenteredNxNLC(nn.Module):
    def __init__(self, hebb_lr, kernel_size, stride, in_size, out_channels, hebb_freq, in_channels, groups):
        super().__init__()
        self.hebb_lr = hebb_lr
        self.kernel_size = tuple_into_int(kernel_size)
        self.stride = tuple_into_int(stride)
        self.in_size = in_size
        self.out_channels = out_channels
        self.hebb_freq = hebb_freq
        self.in_channels = in_channels
        ...
        z_test = torch.ones((1, self.out_channels, *self.in_size))[:, :,
                                                                      self.left_border_h:self.right_border_h,
                                                                      self.left_border_w:self.right_border_w]
        dilation_size = self.kernel_size
        full_size = int(np.ceil(z_test.shape[-1] / dilation_size)) * dilation_size
        lateral_input_counts = torch.conv2d(z_test,
                                                 torch.ones(self.out_channels, 1,
                full_size // dilation_size, full_size // dilation_size),
                                        bias=None, dilation=dilation_size,
                                        padding=(full_size - z_test.shape[-1],
                                                 full_size - z_test.shape[-1]),
                                        groups=self.out_channels)[:, :, (full_size - z_test.shape[-1]):,
                           (full_size - z_test.shape[-1]):]
        self.register_buffer('lateral_input_counts', lateral_input_counts)

    def compute_lateral_connectivity_mean(self, z):
        if self.kernel_size == 1:
            return z.sum(dim=(-2, -1), keepdim=True)
        ...
        dilation_size = self.kernel_size
        full_size = int(np.ceil(z_non_padded.shape[-1] / dilation_size)) * dilation_size
        z_non_padded_mean = torch.conv2d(z_non_padded,
                                        torch.ones(self.out_channels, 1,
                                                   full_size // dilation_size, full_size // dilation_size,
                                                   device=z.device),
                                        bias=None, dilation=dilation_size,
                                        ...
                                        groups=self.out_channels)[:, :, (full_size - z_non_padded.shape[-1]):,
                           (full_size - z_non_padded.shape[-1]):] \
                            / self.lateral_input_counts
        z_non_padded = torch.kron(
            torch.ones(1, 1, full_size // dilation_size, full_size // dilation_size, device=z.device),
            z_non_padded_mean)[:, :, :z_non_padded.shape[-1], :z_non_padded.shape[-1]]
    ...
    def get_b(self):
        b = torch.tile(self.b, (self.hebb_freq, 1, int(np.ceil(self.in_size[0] / self.kernel_size)),
                           int(np.ceil(self.in_size[1] / self.kernel_size))))[
            :, :, :self.in_size[0], :self.in_size[1]]
        b = self.padding(b)
        b = b.unfold(2, self.kernel_size, self.stride).unfold(3, self.kernel_size, self.stride)
        return b.view(b.shape[0], self.groups, self.in_channels // self.groups,
                      b.shape[2], b.shape[3], b.shape[4], b.shape[5])

    def forward(self, x, b, weights):
        self.b = b
        if self.training:
        # noiseless
            self.output = torch.einsum('bgihwkc,igjhwkc->bgjhw', self.get_b(), weights).contiguous()
            self.output = self.output - self.compute_lateral_connectivity_mean(
                self.output.view(x.shape[0], self.out_channels, *self.output.shape[-2:])).view(
                *self.output.shape) + \
                          torch.einsum('bgihwkc,igjhwkc->bgjhw', x, weights).contiguous()
        # # normal
        # self.output = torch.einsum('bgihwkc,igjhwkc->bgjhw', self.b + x, weights).contiguous()
...
class LocallyConnectedNxNHebbWeightSharing(LocallyConnected2d):
    def __init__(self, in_size, in_channels, out_channels, kernel_size, conv_deviation_eps=0.0,
                 groups=1, stride=1, bias=False, hebb_lr=0.1, hebb_freq=8):
        super().__init__(in_size, in_channels, out_channels, kernel_size, conv_deviation_eps, groups, stride, bias)
        self.centered_lc = CenteredNxNLC(hebb_lr, kernel_size, stride, in_size, out_channels,
                                         hebb_freq, in_channels, groups)
        self.hebb_freq = hebb_freq
        self.hebb_lr = hebb_lr
        # self.register_buffer('exp_avg_weights', self.weights.detach().clone())
    ...
    def forward(self, x):
        # x: batch, ch_in, h_out, w_out, k_h, k_w -> z: batch, ch_out, h_out, w_out
        # w: ch_in // groups, ch_out, h_out, w_out, k_h, k_w
        # larger stride doesn't change anything, as lateral grids are independent
        b = torch.randn(int(np.ceil(x.shape[0] / self.hebb_freq)), x.shape[1], self.kernel_size, self.kernel_size,
                        device=x.device)
        x = self.padding(x)
...
def register_hebb_hooks(net):
    ...
    for layer in net.modules():
        if isinstance(layer, LocallyConnected1x1HebbWeightSharing) or \
                isinstance(layer, LocallyConnectedNxNHebbWeightSharing):
