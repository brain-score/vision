...
def get_model_list():
    return ['resnet50_ewcon_wzc_epoch10']

def get_model(name):
    assert name == 'resnet50_ewcon_wzc_epoch10'

    from brainscore_core.supported_data_standards.brainio.s3 import load_file
    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_755/",
                          relative_path="ew_checkpoint_9.pth",
                          version_id="Rr6nr0eqA0QO.6H0qlQpkmRPozfrLOse",
                          ),
    ...
    wrapper = PytorchWrapper(identifier='resnet50_ewcon_wzc_epoch10', model=model, preprocessing=preprocessing)
