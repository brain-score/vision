...
def get_model_list():
    return ['resnet50_no_specularities_iteration=3']

def get_model(name):
    keyword = 'no_specularities'
    iteration = 3
    network = 'resnet50'
    url = 'https://eggerbernhard.ch/shreya/latest_resnet50/no_specularities_3.ckpt'
    output = 'resnet50_no_specularities_iteration=3.ckpt'
    gdown.download(url, output)


    if keyword != 'imagenet_trained' and keyword != 'no_training':
        lx_whole = [f"resnet50_no_specularities_iteration=3.ckpt"]
        if len(lx_whole) > 1:
            lx_whole = [lx_whole[-1]]
    ...
    assert name == 'resnet50_no_specularities_iteration=3'
    url = 'https://eggerbernhard.ch/shreya/latest_resnet50/no_specularities_3.ckpt'
    output = 'resnet50_no_specularities_iteration=3.ckpt'
    gdown.download(url, output)
    layers = []
