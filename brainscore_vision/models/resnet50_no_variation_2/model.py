...
def get_model_list():
    return ['resnet50_no_variation_iteration=2']

def get_model(name):
    keyword = 'no_variation'
    iteration = 2
    network = 'resnet50'
    url = 'https://eggerbernhard.ch/shreya/latest_resnet50/no_variation_2.ckpt'
    output = 'resnet50_no_variation_iteration=2.ckpt'
    gdown.download(url, output)


    if keyword != 'imagenet_trained' and keyword != 'no_training':
        lx_whole = [f"resnet50_no_variation_iteration=2.ckpt"]
        if len(lx_whole) > 1:
            lx_whole = [lx_whole[-1]]
    ...
    assert name == 'resnet50_no_variation_iteration=2'
    url = 'https://eggerbernhard.ch/shreya/latest_resnet50/no_variation_2.ckpt'
    output = 'resnet50_no_variation_iteration=2.ckpt'
    gdown.download(url, output)
    layers = []
