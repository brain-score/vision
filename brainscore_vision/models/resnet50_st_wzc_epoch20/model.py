...
def get_model_list():
    return ['resnet50_st_wzc_epoch20']

def get_model(name):
    assert name == 'resnet50_st_wzc_epoch20'

    from brainscore_vision.model_helpers.s3 import load_file
    file_path = load_file(bucket="brainscore-storage", folder_name="brainscore-vision/models/user_755/",
                          relative_path="checkpoint_19.pth",
                          version_id="_dVzIkEMGvyQenvnivTq5f6ZI407AAu0",
                          ),
    ...
    wrapper = PytorchWrapper(identifier='resnet50_st_wzc_epoch20', model=model, preprocessing=preprocessing)
