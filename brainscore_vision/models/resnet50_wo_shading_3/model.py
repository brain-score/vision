...
def get_model_list():
    return ['resnet50_wo_shading_iteration=3']

def get_model(name):
    keyword = 'wo_shading'
    iteration = 3
    network = 'resnet50'
    url = 'https://eggerbernhard.ch/shreya/latest_resnet50/wo_shading_3.ckpt'
    output = 'resnet50_wo_shading_iteration=3.ckpt'
    gdown.download(url, output)


    if keyword != 'imagenet_trained' and keyword != 'no_training':
        lx_whole = [f"resnet50_wo_shading_iteration=3.ckpt"]
        if len(lx_whole) > 1:
            lx_whole = [lx_whole[-1]]
    ...
    assert name == 'resnet50_wo_shading_iteration=3'
    url = 'https://eggerbernhard.ch/shreya/latest_resnet50/wo_shading_3.ckpt'
    output = 'resnet50_wo_shading_iteration=3.ckpt'
    gdown.download(url, output)
    layers = []
