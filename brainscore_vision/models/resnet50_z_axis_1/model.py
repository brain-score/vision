...
def get_model_list():
    return ['resnet50_z_axis_iteration=1']

def get_model(name):
    keyword = 'z_axis'
    iteration = 1
    network = 'resnet50'
    url = 'https://eggerbernhard.ch/shreya/latest_resnet50/z_axis_1.ckpt'
    output = 'resnet50_z_axis_iteration=1.ckpt'
    gdown.download(url, output)


    if keyword != 'imagenet_trained' and keyword != 'no_training':
        lx_whole = [f"resnet50_z_axis_iteration=1.ckpt"]
        if len(lx_whole) > 1:
            lx_whole = [lx_whole[-1]]
    ...
    assert name == 'resnet50_z_axis_iteration=1'
    url = 'https://eggerbernhard.ch/shreya/latest_resnet50/z_axis_1.ckpt'
    output = 'resnet50_z_axis_iteration=1.ckpt'
    gdown.download(url, output)
    layers = []
