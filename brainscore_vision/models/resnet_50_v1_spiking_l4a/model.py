...
class ResNet50WithSpikingHead(torch.nn.Module):
    def __init__(self):
        ...
        # Wrap each block in layer4 with a spiking neuron
        self.layer4 = torch.nn.Sequential(
            *[torch.nn.Sequential(block, neuron.IFNode()) for block in resnet.layer4]
        )
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)  # Now includes spiking IFNodes
...
def get_model_list():
    return ['resnet_50_v1_spiking_l4a']


def get_model(name):
    assert name == 'resnet_50_v1_spiking_l4a'
    model = ResNet50WithSpikingHead()
    preprocessing = functools.partial(load_preprocess_images, image_size=224)
    wrapper = PytorchWrapper(identifier='resnet_50_v1_spiking_l4a', model=model, preprocessing=preprocessing)
