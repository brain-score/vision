...
model.load_state_dict(torch.load(os.path.join(os.path.dirname(os.path.realpath(__file__)),'model_seed_20_redo.pth'), map_location = device)['state_dict'])
# model = model.module  #Remove DataParallel Wrapper
model.fc_linear = nn.Linear(128,1000) #Imagenet num_classes
print(model)
activations_model = PytorchWrapper(identifier='resnet_blocks_seed20_redo', model=model, preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='resnet_blocks_seed20_redo', activations_model=activations_model,
...
def get_model_list():
    ...
    return ['resnet_blocks_seed20_redo']
...
def get_model(name):
    ...
    """
    assert name == 'resnet_blocks_seed20_redo'
