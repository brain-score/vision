...
class DCN(nn.Module):
    def __init__(self, times = 5):
        ...
        self.initialConv = conv1(3, 32, out_shape = 30)

        self.feature_layer1 = ResBlock(32, 32, out_shape = 30)

        self.DS1 = Downsampling(32, 64, out_shape = 15)

        self.feature_layer2 = ResBlock(64, 64, out_shape = 15)

        self.DS2 = Downsampling(64, 128, out_shape = 7)

        self.feature_layer3 = ResBlock(128, 128, out_shape = 7)
...
model.load_state_dict(torch.load(os.path.join(os.path.dirname(os.path.realpath(__file__)),'resnet_recurrent.pth'), map_location = device)['state_dict'])
# model = model.module  #Remove DataParallel Wrapper
model.fc_linear = nn.Linear(128,1000) #Imagenet num_classes
print(model)
activations_model = PytorchWrapper(identifier='resnet_blocks_recurrent', model=model, preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='resnet_blocks_recurrent', activations_model=activations_model,
...
def get_model_list():
    ...
    return ['resnet_blocks_recurrent']
...
def get_model(name):
    ...
    """
    assert name == 'resnet_blocks_recurrent'
