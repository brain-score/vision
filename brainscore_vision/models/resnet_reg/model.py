...
BIBTEX = """"""
LAYERS = {
    "resnet18_reg": ['conv1', 'layer1.0.conv2', 'layer1.1.conv2', 'layer2.0.downsample.0', 'layer2.1.conv2',
                     'layer3.0.downsample.0', 'layer3.1.conv2', 'layer4.0.downsample.0', 'layer4.1.conv2', 'avgpool'],
    "resnet50_reg": ['conv1', 'layer1.0.conv2', 'layer1.1.conv2', 'layer1.2.conv2', 'layer2.0.downsample.0', 'layer2.1.conv3',
                     'layer2.2.conv2', 'layer2.3.conv2', 'layer3.0.downsample.0', 'layer3.1.conv2', 'layer3.2.conv2',
                     'layer3.3.conv2', 'layer3.4.conv2', 'layer3.5.conv2', 'layer4.0.downsample.0', 'layer4.1.conv2',
                     'layer4.2.conv2', 'avgpool'],
}
...
class AACN_Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        def flatten_hw(x, depth): return torch.reshape(
            x, (batch_size, self.num_heads, height * width, depth))
        ...
        # After splitting, shape is [batch_size, num_heads, height, width, dkh
        # or dvh]
    ...
    def relative_logits(self, q):
        ...
        rel_logits_w = self.relative_logits_1d(
            q, self.rel_embeddings_w, height, width, num_heads, [
                0, 1, 2, 4, 3, 5])
    ...
    def relative_logits_1d(self, q, rel_k, height, width,
                           num_heads, transpose_mask):
        rel_logits = torch.einsum('bhxyd,md->bxym', q, rel_k)
        # Collapse height and heads
        rel_logits = torch.reshape(
            rel_logits, (-1, height, width, 2 * width - 1))
        ...
        rel_logits = torch.reshape(
            rel_logits, (-1, num_heads, height * width, height * width))
        return rel_logits
    ...
    def rel_to_abs(self, x):
        ...
        final_x = torch.reshape(
            flat_x_padded, (batch_size, num_heads, L + 1, 2 * L - 1))
...
def get_model(name):
    ...
    """
    assert name in model_constructors

    model_fn = model_constructors[name]
    model = model_fn(
        num_classes=8,
        attention=[
            False,
    ...
    activations_model = PytorchWrapper(identifier=name, model=model,
