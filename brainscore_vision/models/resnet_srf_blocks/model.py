...
model.load_state_dict(torch.load(os.path.join(os.path.dirname(os.path.realpath(__file__)),'srf_blocks.pth'), map_location = device)['state_dict'])
# model = model.module  #Remove DataParallel Wrapper
model.fc_linear = nn.Linear(128,1000) #Imagenet num_classes
print(model)
activations_model = PytorchWrapper(identifier='resnet_srf_blocks_adv_cifar10', model=model, preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='resnet_srf_blocks_adv_cifar10', activations_model=activations_model,
...
def get_model_list():
    ...
    return ['resnet_srf_blocks_adv_cifar10']
...
def get_model(name):
    ...
    """
    assert name == 'resnet_srf_blocks_adv_cifar10'
