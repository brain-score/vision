...
model.load_state_dict(torch.load(os.path.join(os.path.dirname(os.path.realpath(__file__)),'resnet_srf_blocks.pth'), map_location = device)['state_dict'])
model = model.module  #Remove DataParallel Wrapper
model.fc_linear = nn.Linear(128,1000) #Imagenet num_classes
print(model)
activations_model = PytorchWrapper(identifier='resnet_srf_blocks_mar31_tinyImagenet', model=model, preprocessing=preprocessing)

# actually make the model, with the layers you want to see specified:
model = ModelCommitment(identifier='resnet_srf_blocks_mar31_tinyImagenet', activations_model=activations_model,
                        # specify layers to consider
                        layers=['initialConv', 'feature_layer1.conv2', 'feature_layer2.conv2', 'feature_layer3.conv2'])
...
def get_model_list():
    ...
    return ['resnet_srf_blocks_mar31_tinyImagenet']
...
def get_model(name):
    ...
    """
    assert name == 'resnet_srf_blocks_mar31_tinyImagenet'
...
def get_layers(name):
    ...
    return ['initialConv', 'feature_layer1.conv2', 'feature_layer2.conv2', 'feature_layer3.conv2']
