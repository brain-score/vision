...
def get_model(name):
    ...
    ckpt = torch.load(os.path.abspath(os.path.join(dir_path, 'resnet50_alpha0p9.pth')), map_location='cpu')
    ckpt['fc.weight'] = model.fc.weight
    ckpt['fc.bias'] = model.fc.bias
    model.load_state_dict(ckpt)
    preprocessing = functools.partial(load_preprocess_images, image_size=224)
    wrapper = PytorchWrapper(identifier='resnet50-vicregl0p9', model=model, preprocessing=preprocessing)
