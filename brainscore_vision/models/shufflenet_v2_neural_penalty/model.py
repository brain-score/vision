...
def get_model(name):
    assert name == 'shufflenet_v2_neural_penalty'
    model = load_model()
    preprocessing = functools.partial(load_preprocess_images, image_size=224)
    wrapper = PytorchWrapper(identifier='shufflenet_v2_neural_penalty', model=model, preprocessing=preprocessing)
...
def get_bibtex(model_identifier):
    return """
    @misc{shufflenet_v2_neural_penalty,
