    ...
    def __init__(self, in_channel, rep_channel, kernel_size, stride, padding_mode, nonlinearity, softplus_beta,
        ...
        if opposite_pair:
            ...
            self._weight_encode = nn.Parameter(
                torch.randn([rep_channel // 2, in_channel, kernel_size, kernel_size]) / (kernel_size * np.sqrt(rep_channel)))
            self._bias_encode = nn.Parameter(torch.zeros([rep_channel // 2])) if allow_bias else None
        else:
            self._weight_encode = nn.Parameter(
                torch.randn([rep_channel, in_channel, kernel_size, kernel_size]) / (kernel_size * np.sqrt(rep_channel)))
            self._bias_encode = nn.Parameter(torch.zeros([rep_channel])) if allow_bias else None
            if not share_weight:
...
class PURENetLayer(URENetLayer):
    def __init__(self, in_channel, rep_channel, kernel_size, stride, padding_mode, nonlinearity, softplus_beta,
                 skip_connection, allow_bias, share_weight, n_negated, opposite_pair, magnitude):
        super().__init__(in_channel, rep_channel, kernel_size, stride, padding_mode, nonlinearity, softplus_beta,
                         skip_connection, allow_bias, share_weight, n_negated, opposite_pair)
    ...
    def learn_preprocess_parameters(self, train_data):
        self.register_buffer("preprocess_gain", self.magnitude / (train_data.mean(dim=(0, 2, 3), keepdim=True) + 1e-2))
        self.register_buffer("preprocess_bias", torch.zeros_like(train_data).sum(dim=(0, 2, 3), keepdim=True))
...
class PURENetHierarchy(URENetHierarchy):
    def get_layer_class(self):
        return PURENetLayer
