...
class GT_encoder(nn.Module):
    ...
    def get_segmentation(self, x):
        ...
        result = self.image_processor.post_process_panoptic_segmentation(outputs,
                                                                    target_sizes=[list(x.shape[-2:])] * x.shape[
                                                                        0])  # [0]

        all_results = []
        for res in result:
            if res['segmentation'].device != x.device:
                res['segmentation'] = res['segmentation'].to(x.device)
            all_results.append(res['segmentation'])

        all_results = torch.stack(all_results, 0)
...
class GTModel(nn.Module):
    def __init__(self, model_name):
        ...
        self.encoder = GT_encoder(model_name)
