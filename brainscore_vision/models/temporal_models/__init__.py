...
def commit_model(identifier):
    activations_model=torchvision_models.get_model(identifier)
    layers=get_specified_layers(activations_model)
    region_layer_map={"IT": layers[-2]}  # A hack here: we assume the second last layer is IT now, will provide a better way to specify this later
