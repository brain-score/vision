...
def get_model_list():
    return ['tf_efficientnetv2_s_in21ft1k_robust_linf_400x400']
...
def get_model(name):
    assert name == 'tf_efficientnetv2_s_in21ft1k_robust_linf_400x400'
    model= EffNetBX()
    dir_path = os.path.dirname(os.path.realpath(__file__))
    state_dict = torch.load(dir_path + "/tf_efficientnetv2_s_in21ft1k_robust_linf_untargeted0_checkpoint.pt", map_location=torch.device('cpu'))["model"]
