...
def get_model_list():
    return ['tf_efficientnet_b1_ns_robust_linf4255_324x288']
...
def get_model(name):
    assert name == 'tf_efficientnet_b1_ns_robust_linf4255_324x288'
    model= EffNetBX()
    dir_path = os.path.dirname(os.path.realpath(__file__))
    state_dict = torch.load(dir_path + "/tf_efficientnet_b1_ns_robust_linf_4_255_untargeted0_checkpoint.pt", map_location=torch.device('cpu'))["model"]
