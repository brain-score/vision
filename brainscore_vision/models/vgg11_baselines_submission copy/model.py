...
weights_path = Path(__file__).parent / 'supervised.pt'
state_dict = torch.load(weights_path)
supervised_network.load_state_dict(state_dict, strict=False)

lpl_e2e_network = VGG11Encoder()
weights_path = Path(__file__).parent / 'lpl_e2e.pt'
state_dict = torch.load(weights_path)
lpl_e2e_network.load_state_dict(state_dict, strict=False)

neg_samples_network = VGG11Encoder(projector_mlp=True)
weights_path = Path(__file__).parent / 'neg_samples.pt'
state_dict = torch.load(weights_path)
neg_samples_network.load_state_dict(state_dict, strict=False)

# get an activations model from the Pytorch Wrapper
activations_model_supervised = PytorchWrapper(identifier='supervised', model=supervised_network, preprocessing=preprocessing)
activations_model_lpl_e2e = PytorchWrapper(identifier='lpl_e2e', model=lpl_e2e_network, preprocessing=preprocessing)
activations_model_neg_samples = PytorchWrapper(identifier='neg_samples', model=neg_samples_network, preprocessing=preprocessing)
...
def get_model_list():
    ...
    return ['supervised', 'lpl_e2e', 'neg_samples']
