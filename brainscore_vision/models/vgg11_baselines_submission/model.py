...
weights_path = Path(__file__).parent / 'supervised_e2e.pt'
state_dict = torch.load(weights_path)
supervised_e2e_network.load_state_dict(state_dict, strict=False)

lpl_network = VGG11Encoder()
weights_path = Path(__file__).parent / 'lpl.pt'
state_dict = torch.load(weights_path)
lpl_network.load_state_dict(state_dict, strict=False)

# get an activations model from the Pytorch Wrapper
activations_model_untrained = PytorchWrapper(identifier='untrained', model=untrained_network, preprocessing=preprocessing)
activations_model_supervised_e2e = PytorchWrapper(identifier='supervised_e2e', model=supervised_e2e_network, preprocessing=preprocessing)
activations_model_lpl = PytorchWrapper(identifier='lpl', model=lpl_network, preprocessing=preprocessing)
...
def get_model_list():
    ...
    return ['untrained', 'supervised_e2e', 'lpl']
