...
weights_path = Path(__file__).parent / 'cifar_supervised_e2e.pt'
state_dict = torch.load(weights_path)
supervised_e2e_network.load_state_dict(state_dict, strict=False)

supervised_network = VGG11Encoder()
weights_path = Path(__file__).parent / 'cifar_supervised.pt'
state_dict = torch.load(weights_path)
supervised_network.load_state_dict(state_dict, strict=False)

lpl_e2e_network = VGG11Encoder()
weights_path = Path(__file__).parent / 'cifar_lpl_e2e.pt'
state_dict = torch.load(weights_path)
lpl_e2e_network.load_state_dict(state_dict, strict=False)

lpl_network = VGG11Encoder()
weights_path = Path(__file__).parent / 'cifar_lpl.pt'
state_dict = torch.load(weights_path)
lpl_network.load_state_dict(state_dict, strict=False)

neg_samples_e2e_network = VGG11Encoder()
weights_path = Path(__file__).parent / 'cifar_neg_samples_e2e.pt'
state_dict = torch.load(weights_path)
neg_samples_e2e_network.load_state_dict(state_dict, strict=False)

neg_samples_network = VGG11Encoder(projector_mlp=True)
weights_path = Path(__file__).parent / 'cifar_neg_samples.pt'
state_dict = torch.load(weights_path)
neg_samples_network.load_state_dict(state_dict, strict=False)

# get an activations model from the Pytorch Wrapper
activations_model_untrained = PytorchWrapper(identifier='untrained_cifar', model=untrained_network, preprocessing=preprocessing)

activations_model_supervised_e2e = PytorchWrapper(identifier='supervised_e2e_cifar', model=supervised_e2e_network, preprocessing=preprocessing)
activations_model_supervised = PytorchWrapper(identifier='supervised_cifar', model=supervised_network, preprocessing=preprocessing)

activations_model_lpl_e2e = PytorchWrapper(identifier='lpl_e2e_cifar', model=lpl_e2e_network, preprocessing=preprocessing)
activations_model_lpl = PytorchWrapper(identifier='lpl_cifar', model=lpl_network, preprocessing=preprocessing)

activations_model_neg_samples_e2e = PytorchWrapper(identifier='neg_samples_e2e_cifar', model=neg_samples_e2e_network, preprocessing=preprocessing)
activations_model_neg_samples = PytorchWrapper(identifier='neg_samples_cifar', model=neg_samples_network, preprocessing=preprocessing)
...
def get_model_list():
    ...
    return ['untrained_cifar', 'supervised_e2e_cifar', 'supervised_cifar', 'lpl_e2e_cifar', 'lpl_cifar', 'neg_samples_e2e_cifar', 'neg_samples_cifar']
