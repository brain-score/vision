...
device = "cpu"
keyword = 'no_specularities'
iteration = 1
network = 'vgg16'
url = f'https://eggerbernhard.ch/shreya/vgg16_no_specularities_iteration=1.ckpt'
output = f'vgg16_no_specularities_iteration=1.ckpt'
gdown.download(url, output)

if keyword != 'imagenet_trained' and keyword != 'no_training':
    lx_whole = list(f"vgg16_no_specularities_iteration=1.ckpt")
    if len(lx_whole) > 1:
        lx_whole = [lx_whole[-1]]
...
def get_model_list():
    return [f'vgg16_no_specularities_iteration=1']

def get_model(name):
    assert name == f'vgg16_no_specularities_iteration=1'
    url = f'https://eggerbernhard.ch/shreya/vgg16_no_specularities_iteration=1.ckpt'
    output = f'vgg16_no_specularities_iteration=1.ckpt'
    gdown.download(url, output)
...
def get_layers(name):
    assert name == f'vgg16_no_specularities_iteration=1.ckpt'
    layers = []
    url = f'https://eggerbernhard.ch/shreya/vgg16_no_specularities_iteration=1.ckpt'
    output = f'https://eggerbernhard.ch/shreya/vgg16_no_specularities_iteration=1.ckpt'
    gdown.download(url, output)
    for name, module in model.named_modules():
        layers.append(name)
    return layers
