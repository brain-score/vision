...
def generate_gabor_param(
    features, seed=0, rand_flag=False, sf_corr=0, sf_max=9, sf_min=0
):
    ...
    if rand_flag:
        ...
        nx = np.interp(
            samps_cdf[:, 0], np.hstack(([0], nx_dist.cumsum())), np.log10(nx_bins)
        )
        nx = 10**nx
        ...
        sf = np.interp(
            samps_cdf[:, 1], np.hstack(([0], sf_dist.cumsum())), np.log2(sf_bins)
        )
        sf = 2**sf
...
class SKConv(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, M=2, r=16, L=32, groups=32):
        ...
        for i in range(M):
            ...
            self.conv.append(
                nn.Sequential(
                    conv1, nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True)
                )
            )
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        conv_fc = nn.Conv2d(out_channels, d, 1, bias=False)
        init.normal_(conv_fc.weight, std=0.01)
        self.fc1 = nn.Sequential(conv_fc, nn.BatchNorm2d(d), nn.ReLU(inplace=True))
...
class GRCL(nn.Module):
    def __init__(
        self, inplanes, planes, downsample=True, iter=3, SKconv=True, expansion=2
    ):
        ...
        if self.downsample:
            ...
            if SKconv:
                ...
                d_conv_3 = nn.Conv2d(
                    planes, planes, kernel_size=3, stride=2, padding=1, bias=False
                )
    ...
    def forward(self, x):
        ...
        for i in range(0, self.iter):
            x_g_r = self.conv_g_r(
                self.__dict__["_modules"]["iter_g_%s" % str(i + 1)](x_s)
            )
            x_s = (
                self.__dict__["_modules"]["iter_%s" % str(i + 1)](x_s)
                * torch.sigmoid(x_g_r + x_g_s)
                + x_s
            )
        ...
        if self.downsample:
            ...
            x_s_2 = self.d_conv_1e(
                self.d_conv_3(
                    self.d_relu_3(
                        self.d_bn_3(self.d_conv_1(self.d_relu_1(self.d_bn_1(x_s))))
                    )
                )
            )
            x_s = x_s_1 + x_s_2
...
class GRCNNBackEnd(nn.Module):
    def __init__(self, iters, maps, SKconv, expansion, num_classes):
        ...
        self.layer1 = GRCL(
            64, self.maps[0], True, self.iters[0], SKconv, self.expansion
        )
        self.layer2 = GRCL(
            self.maps[0] * self.expansion,
            self.maps[1],
            True,
            self.iters[1],
            SKconv,
        ...
        self.lastact = nn.Sequential(
            nn.BatchNorm2d(self.maps[3] * self.expansion), nn.ReLU(inplace=True)
        )
...
download_weights(
    ...
    folder_path="models/vonegrcnn_47e",
    filename_version_sha=[
        (
            "model_best.pth",
            "KgdVyjmNKqhhfsAWkQAfiJYlvNiK8iPv",
            "b6f41387ad77c1565aacb19074a02a770eb27884",
        )
    ],
...
all_layers = [
    ...
    "module.bottleneck",
    "module.model.layer2",
    "module.model.layer2.conv_g_r",
    "module.model.layer2.iter_1.8",
    "module.model.layer2.iter_g_1.1",
    "module.model.layer2.iter_2.8",
    "module.model.layer2.iter_g_2.1",
    "module.model.layer2.iter_3.8",
    "module.model.layer2.iter_g_3.1",
    "module.model.layer3",
    "module.model.layer3.conv_f",
    "module.model.layer3.conv_g_r",
    "module.model.layer3.iter_1.8",
    "module.model.layer3.iter_g_1.1",
    "module.model.layer3.iter_2.8",
    "module.model.layer3.iter_g_2.1",
    "module.model.layer3.iter_3.8",
    "module.model.layer3.iter_g_3.1",
    "module.model.layer3.iter_4.8",
    "module.model.layer3.iter_g_4.1",
    "module.model.layer3.d_conv_1e",
    "module.model.layer4",
    "module.model.layer4.conv_g_r",
    "module.model.layer4.iter_1.8",
    "module.model.layer4.iter_g_1.1",
    "module.model.layer4.iter_2.8",
    "module.model.layer4.iter_g_2.1",
    "module.model.layer4.iter_3.8",
