...
class SKConv(nn.Module):
    def __init__(self, in_channels, out_channels,
                 stride=1, M=2, r=16, L=32, groups=32):
        ...
        self.fc1 = nn.Sequential(
            conv_fc, nn.BatchNorm2d(d), nn.ReLU(
                inplace=True))
...
class GRCL(nn.Module):
    def __init__(
        ...
        for i in range(0, self.iter):
            ...
            self.add_module("iter_g_" + str(i + 1),
                            nn.Sequential(*layers_g_bn))
...
class GRCNNBackEnd(nn.Module):
    def __init__(self, iters, maps, SKconv, expansion, num_classes):
        ...
        self.lastact = nn.Sequential(
            nn.BatchNorm2d(self.maps[3] *
                           self.expansion), nn.ReLU(inplace=True)
        )
...
def load_model():
    ...
    download_weights(
        ...
        folder_path="models/vonegrcnn_52e_full",
        filename_version_sha=[
            (
                "model_best.pth",
                "u9w_i4Nl3jMBdWqHiYlwJviy2u0cXFf8",
                "28c4bbef948b2bed1df79273e44b0f65094481a9",
            )
        ],
