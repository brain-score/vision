...
class Yolos_Small_PytorchWrapper(PytorchWrapper):
    ...
    def register_hook(self, layer, layer_name, target_dict):
        def hook_function(_layer, _input, output, name=layer_name):
            if isinstance(output, tuple):
                output = output[0]
            target_dict[name] = Yolos_Small_PytorchWrapper._tensor_to_numpy(output)
...
def get_model(name):
    ...
    wrapper = Yolos_Small_PytorchWrapper(identifier=name, model=model, preprocessing=preprocessing)
