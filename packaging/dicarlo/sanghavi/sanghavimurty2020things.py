...
def collect_stimuli(data_dir):
    ...
    stimuli1 = pd.read_csv(data_dir / 'image-metadata' / 'things_1_metadata.csv')

    stimuli1 = stimuli1.rename(columns={'id': 'image_id'})

    stimuli1['image_current_local_file_path'] = stimuli1.apply(
        lambda row: os.path.join(image_dir, str(row.image_id) + '.jpg'), axis=1)

    assert len(np.unique(stimuli1['image_id'])) == len(stimuli1)
    ...
    assert os.path.isdir(data_dir / 'image-metadata')
    stimuli2 = pd.read_csv(data_dir / 'image-metadata' / 'things_2_metadata.csv')

    stimuli2 = stimuli2.rename(columns={'id': 'image_id'})

    stimuli2['image_current_local_file_path'] = stimuli2.apply(
        lambda row: os.path.join(image_dir, str(row.image_id) + '.jpg'), axis=1)

    assert len(np.unique(stimuli2['image_id'])) == len(stimuli2)

    stimuli = StimulusSet(pd.concat([stimuli1, stimuli2], ignore_index=True))
    stimuli.image_paths = \
...
def load_responses(data_dir, stimuli):
    ...
    psth1 = np.load(data_dir / 'solo.rsvp.things-1.experiment_psth.npy')  # Shaped images x repetitions x time_bins x channels
    # print(psth1.shape)
    psth2 =np.load(data_dir / 'solo.rsvp.things-2.experiment_psth.npy')  # Shaped images x repetitions x time_bins x channels
    # print(psth2.shape)
    ...
    psth1 = np.load(data_dir / 'solo.rsvp.things-1.normalizer_psth.npy')
    psth2 = np.load(data_dir / 'solo.rsvp.things-2.normalizer_psth.npy')
    psth = np.hstack((psth1, psth2))
...
def main():
    ...
    stimuli.identifier = 'dicarlo.THINGS'
