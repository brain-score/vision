...
import brainscore
import brainscore.fetch
import brainscore.lookup
from brainscore import assemblies, fetch

_hvm_s3_url = "https://mkgu-dicarlolab-hvm.s3.amazonaws.com/hvm_neuronal_features.nc"


def test_nr_assembly_ctor():
    assy_hvm = brainscore.get_assembly(name="dicarlo.Majaj2015")
    assert isinstance(assy_hvm, brainscore.assemblies.DataAssembly)
...
def test_lookup():
    assy = brainscore.assemblies.lookup_assembly("dicarlo.Majaj2015")
...
def test_lookup_bad_name():
    with pytest.raises(brainscore.assemblies.AssemblyLookupError) as err:
        brainscore.assemblies.lookup_assembly("BadName")


def test_fetch():
    assy_record = brainscore.assemblies.lookup_assembly("dicarlo.Majaj2015")
