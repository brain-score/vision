...
1) Fill in correct values for ceilings, model scores
2) Capture models .nc files: locate the cached (pickled) activations 
   (likely in ~/.result_caching/model_tools.activations.core.ActionsExtractorHelper._from_paths_stored/....pkl
3) Convert those activations to netCDF: convert pickled activations into netcdf:
...
3) Upload those to S3
4) Update test__init__ files
5) Add filename to test_setup.sh
