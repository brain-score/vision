...
def test_score_precomputed_alexnet_activations():
    # load activations
    activations = pd.read_pickle(os.path.join(os.path.dirname(__file__), 'alexnet-pca200.pkl'))['data']
    activations = NeuroidAssembly(activations)
    # score
    benchmark = benchmarks.load('dicarlo.Majaj2015')
    scores = benchmark(activations, source_splits=['layer'])
    # eval
    region_best_layers = {'V4': 'conv1', 'IT': 'conv5'}
    for region in region_best_layers:
        score = scores.aggregation.sel(region=region, aggregation='center')
        max_region = score['layer'][score.argmax()]
        assert max_region['layer'] == region_best_layers[region]
