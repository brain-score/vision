...
class TestI2N(object):
    def test_objectome(self):
        ...
        # subsample = pd.read_pickle(os.path.join(os.path.dirname(__file__), 'objectome24s100_imgsubsampled240_pandas.pkl'))
        # objectome = objectome[objectome['id'].isin(subsample.values[:, 0])]
        # objectome.to_pickle('objectome240.pkl')

        objectome = pd.read_pickle(os.path.join(os.path.dirname(__file__), 'objectome240.pkl'))
        ...
        # np.testing.assert_array_equal(len(grouped), [240 * 24])
