...
class TestRunScoring:
    ...
    def setup_method(self):
        ...
        database_models.User.create(id=self.unique_id, email=f'test_{self.unique_id}@brainscore.com', password='abcde',
                                    is_active=True, is_staff=False, is_superuser=False, last_login='2022-10-14 9:25:00')
    
    def get_model_entries(self):
        submission = database_models.Submission.get(database_models.Submission.jenkins_id == self.unique_id)
        model_entries = database_models.Model.select().where(database_models.Model.submission_id == submission.id)
        return model_entries
    
    def get_score_entries(self):
        submission = database_models.Submission.select(database_models.Submission.id).where(database_models.Submission.jenkins_id == self.unique_id).get()
        model_ids = database_models.Model.select(database_models.Model.id).where(database_models.Model.submission_id == submission.id)
        score_entries = list(database_models.Score.select().where(database_models.Score.model_id.in_(model_ids)))
        return score_entries

    def test_successful_run(self):
        args_dict = {'jenkins_id': self.unique_id, 'user_id': self.unique_id, 'model_type': 'brainmodel',
    ...
    def test_competition_field_set(self):
        ...
        run_scoring(args_dict)
        model_entries = self.get_model_entries()
